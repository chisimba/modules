...
class xajax {
    ...
    // usage: $xajax->setRequestURI("<http://xajax.sourceforge.net%22>;);
    ...
    function registerFunction($sFunction, $sRequestType = POST)
    {
        if (is_array($sFunction)) {
            $this->aFunctionReferences[$sFunction[1]] = $sFunction;
            $this->aFunctions[] = $sFunction[1];
            $this->aFunctionRequestTypes[$sFunction[1]] = $sRequestType;
        } else {
    ...
    function processRequests()
    {
        ...
        if (!in_array($sFunctionName, $this->aFunctions)) {
            ...
            if (is_array($this->aFunctionReferences[$sFunctionName])) {
                $sResponse = call_user_func_array($this->aFunctionReferences[$sFunctionName], $aArgs);
            } else {
    ...
    function compressJavascript($sJS)
    {
        ...
        function generateJavascript()
        {
            $js = "";
            if ($this->bDebug) {
                $js .= "var xajaxDebug=" . ($this->bDebug?"true":"false") . ";\n";
            }
...
{
<?php if ($this->bDebug) {
                ?>this.DebugMessage("Initializing Request Object..");<?php }
            ?>
var req;
try
{
...
<?php if ($this->bDebug) {
                ?>if (!req) this.DebugMessage("Request Object Instantiation failed.");<?php }
            ?>
...
<?php if ($this->bStatusMessages == true) {
                ?>window.status = 'Sending Request...';<?php }
            ?>
<?php if ($this->bDebug) {
                ?>this.DebugMessage("Starting xajax...");<?php }
            ?>
...
r.open(xajaxRequestType==<?php print GET;
            ?>?"GET":"POST", uri, true);
...
<?php if ($this->bDebug) {
                ?>this.DebugMessage("Calling "+sFunction +" uri="+uri+" (post:"+ postData +")");<?php }
            ?>
r.send(postData);
<?php if ($this->bStatusMessages == true) {
                ?>window.status = 'Waiting for data...';<?php }
            ?>
<?php if ($this->bDebug) {
                ?>this.DebugMessage(sFunction + " waiting..");<?php }
            ?>
delete r;
return true;
}
...
{
<?php if ($this->bStatusMessages == true) {
                ?> window.status = 'Recieving data...'; <?php }
            ?>
var tmpXajax = null;
