...
class jsondecoder
{
     ...
     * @param int $decodeType How objects should be decoded
     * @return void
     */
    protected function __construct($source, $decodeType)
    {
        ...
        if (!in_array($decodeType, array(jsonserializer::TYPE_ARRAY, jsonserializer::TYPE_OBJECT )))
        {
            $decodeType = jsonserializer::TYPE_ARRAY;
        }
     ...
     * return a StdClass object instead, pass {@link jsonserializer::TYPE_OBJECT} to
     * the $objectDecodeType parameter.
     *
     * Throws a customException if the source string is null.
     *
     * @static
     * @access public
     * @param string $source String to be decoded
     * @param int $objectDecodeType How objects should be decoded; should be
     * either or {@link jsonserializer::TYPE_ARRAY} or
     * {@link jsonserializer::TYPE_OBJECT}; defaults to TYPE_ARRAY
     * @return mixed
     * @throws customException
     */
    public static function decode($source = null, $objectDecodeType = jsonserializer::TYPE_ARRAY)
    {
        if (null === $source) {
            throw new customException('Must specify JSON encoded source for decoding');
        } elseif (!is_string($source)) {
            throw new customException('Can only decode JSON encoded strings');
        }
     ...
     *
     * If jsonenoder was used to encode the original object
    ...
    protected function _decodeObject()
    {
    	...
    	while ($tok && $tok != self::RBRACE) {
    	    if ($tok != self::DATUM || ! is_string($this->_tokenValue)) {
        		throw new customException('Missing key in object encoding: ' . $this->_source);
    	    }
    	    ...
    	    if ($tok != self::COLON) {
        		throw new customException('Missing ":" in object encoding: ' . $this->_source);
        	}
        ...
        switch ($this->_decodeType) {
            case jsonserializer::TYPE_OBJECT:
                ...
                break;
            case jsonserializer::TYPE_ARRAY:
            default:
                $result = $members;
                break;
        }
    ...
    protected function _decodeArray()
    {
    	...
    	while ($tok && $tok != self::RBRACKET) {
    	    ...
    	    if ($tok != self::COMMA) {
        		throw new customException('Missing "," in array encoding: ' . $this->_source);
    	    }
            		...
            		if ($chr == '\\') {
            		    ...
            		    switch ($chr) {
                		    ...
                		    case 't' :
                    		    ...
                    		    break;
                		    default:
                    			throw new customException("Illegal escape "
                                    .  "sequence '" . $chr . "'");
                		    }
            		} elseif ($chr == '"') {
            		    break;
    	...
    	if ($chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9')) {
    	    if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s',
        		...
        		if (is_numeric($datum)) {
        		    ...
        		    throw new customException("Illegal number format: $datum");
        		}
