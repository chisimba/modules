...
class SparqlEngineDb_ResultRenderer_XML implements SparqlEngineDb_ResultRenderer {
	 ...
	 *   @var array
	 */
	protected $arCreationMethods = array ('s' => 'createSubjectFromDbRecordSetPart', 'p' => 'createPredicateFromDbRecordSetPart', 'o' => 'createObjectFromDbRecordSetPart' );
	...
	public function convertFromDbResults($arRecordSets, Query $query, SparqlEngineDb $engine) {
		...
		switch ($strResultForm) {
			...
			case 'construct' :
			case 'describe' :
				throw new Exception ( 'Construct and describe are not supported by the' . ' XML renderer' );
			...
			case 'ask' :
				...
				if ($strResultForm == 'ask') {
					$strCode = $this->getHead () . '  <boolean>' . self::getSpokenBoolean ( $nCount > 0 ) . '</boolean>';
				} else {
					$strCode = $this->getHead () . '  <int>' . $nCount . '</int>';
				}
				break;
...
EOT . $strCode . "\n" . "</sparql>\n";
	...
	protected function createFromRecords($arRecordSets, $strResultForm) {
		...
		foreach ( $arRecordSets as $dbRecordSet ) {
			...
			if ($dbRecordSet->RowCount () <= 0) {
				...
				return $strHeadXml . '  <results ordered="' . self::getSpokenBoolean ( $arSM ['order by'] !== null ) . '" distinct="' . self::getSpokenBoolean ( $strResultForm == 'select distinct' ) . '">' . "\n" . '    <!-- empty result -->' . PHP_EOL . "  </results>\n";
			}
		...
		return $strHeadXml . '  <results ordered="' . self::getSpokenBoolean ( $arSM ['order by'] !== null ) . '" distinct="' . self::getSpokenBoolean ( $strResultForm == 'select distinct' ) . '">' . "\n" . $this->getResultXml ( $arResult ) . "  </results>\n";
	...
	protected function getResultXml($arResult) {
		...
		foreach ( $arResult as $arSet ) {
			...
			foreach ( $arSet as $strVarName => $strValue ) {
				if ($strValue !== null) {
					$strCode .= '      <binding name="' . substr ( $strVarName, 1 ) . '">' . $strValue . "</binding>\n";
				}
			}
			$strCode .= "    </result>\n";
		}
	...
	protected function createSubjectFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase, $strVar) {
		...
		if ($dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_is']] == 'r' || //null should be predicate which is always a resource
	...
	protected function createObjectFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase, $strVar) {
		...
		switch ($dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_is']]) {
			...
			case 'b' :
				...
				break;
			default :
				return $this->getXmlLiteral ( $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_value']], $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_lang']], $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_type']] );
		}
	} //protected function createObjectFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase, $strVar)
