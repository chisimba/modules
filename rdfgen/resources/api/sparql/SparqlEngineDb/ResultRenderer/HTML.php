...
class SparqlEngineDb_ResultRenderer_HTML implements SparqlEngineDb_ResultRenderer {
	...
	protected function wrapCode($strCode) {
		...
		return '<div class="SparqlEngineDb_ResultRenderer_HTML_result">' . "\n" . $strCode . "\n" . "</div>\n";
	} //protected function wrapCode($strCode)
	...
	protected function createTableFromRecords($arRecordSets) {
			...
			foreach ( $dbRecordSet as $row ) {
				...
				foreach ( $arResultVars as $strVar ) {
					...
					if (! isset ( $this->sg->arVarAssignments [$strVarName] )) {
						...
						$arResultRow [$strVarName] = '<td style="background-color: ' . $strColor . '">' . $strCode . '</td>';
					}
				}
		...
		return "<table border='1'>\n" . " <caption>SPARQL result with " . count ( $arResult ) . " rows</caption>\n" . " <thead><th>" . implode ( '</th><th>', array_keys ( reset ( $arResult ) ) ) . "</th></thead>\n" . " <tbody>\n  <tr>" . implode ( "</tr>\n  <tr>", array_map ( create_function ( '$ar', 'return implode("", $ar);' ), $arResult ) ) . "</tr>\n </tbody>\n" . "</table>\n";
	} //protected function createTableFromRecords($arRecordSets)
	...
	protected function createObjectFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase, $strVar) {
		...
		switch ($dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_is']]) {
			...
			case 'b' :
				...
				break;
			default :
				return $this->getHtmlLiteral ( $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_value']], $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_lang']], $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_type']] );
		}
	} //protected function createObjectFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase, $strVar)
	...
	protected function getHtmlResource($value) {
		return array (htmlspecialchars ( $value ), HTML_TABLE_RESOURCE_COLOR );
	} //protected function getHtmlResource($value)
