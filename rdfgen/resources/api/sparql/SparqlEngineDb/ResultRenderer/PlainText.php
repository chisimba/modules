...
class SparqlEngineDb_ResultRenderer_PlainText implements SparqlEngineDb_ResultRenderer {
	...
	protected function getVariableArrayFromRecordSets($arRecordSets, $strResultForm) {
		...
		foreach ( $arRecordSets as $dbRecordSet ) {
			$arResult = array_merge ( $arResult, $this->getVariableArrayFromRecordSet ( $dbRecordSet, $strResultForm ) );
		}
		return $arResult;
	} //protected function getVariableArrayFromRecordSets($arRecordSets, $strResultForm)
	...
	protected function createStatementFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase) {
		return new Statement ( $this->createSubjectFromDbRecordSetPart ( $dbRecordSet, $strVarBase ), $this->createPredicateFromDbRecordSetPart ( $dbRecordSet, $strVarBase ), $this->createObjectFromDbRecordSetPart ( $dbRecordSet, $strVarBase ) );
	} //protected function createStatementFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase)
	...
	protected function createObjectFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase, $strVar) {
		...
		switch ($dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_is']]) {
			...
			case 'b' :
				...
				break;
			default :
				return $this->createLiteral ( $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_value']], $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_lang']], $dbRecordSet->fields [$strVarBase . '.' . $this->sg->arVarAssignments [$strVarName] ['sql_type']] );
		}
	} //protected function createObjectFromDbRecordSetPart(ADORecordSet $dbRecordSet, $strVarBase, $strVar)
