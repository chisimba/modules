	...
	public static function convertFromDbResults($arRecordSets, SparqlEngineDb $engine, $resultform = false) {
		if (is_object ( $resultform )) {
			if ($resultform instanceof SparqlEngineDb_ResultRenderer) {
				return $resultform->convertFromDbResults ( $arRecordSets, $engine->getQuery (), $engine );
			} else {
				throw new Exception ( 'Result renderer object needs to implement' . ' SparqlEngineDb_ResultRenderer interface' );
			}
		}
		...
		if ($strClass = self::loadClass ( $resultform )) {
			...
			if ($rrObj instanceof SparqlEngineDb_ResultRenderer) {
				return $rrObj->convertFromDbResults ( $arRecordSets, $engine->getQuery (), $engine );
			} else {
				throw new Exception ( 'Result renderer class "' . $strClass . '" needs to implement' . ' SparqlEngineDb_ResultRenderer interface' );
			}
		} else {
			throw new Exception ( 'Result renderer class "' . $resultform . '" could not be loaded.' );
		}
	} //public static function convertFromDbResults($arRecordSets, SparqlEngineDb $engine, $resultform = false)
