...
class SparqlEngineDb_FilterGenerator {
	 ...
	 *   @var array
	 */
	protected static $arFuncParamNumbers = array ('bound' => array (1, 1 ), 'datatype' => array (1, 1 ), 'isblank' => array (1, 1 ), 'isiri' => array (1, 1 ), 'isliteral' => array (1, 1 ), 'isuri' => array (1, 1 ), 'lang' => array (1, 1 ), 'langmatches' => array (2, 2 ), 'regex' => array (2, 3 ), 'sameterm' => array (2, 2 ), 'str' => array (1, 1 ), 'xsd:datetime' => array (1, 1 ) );
	...
	protected function createTreeSql($tree, $parent) {
		switch ($tree ['type']) {
			...
			case 'value' :
				if ($parent != null) {
					$bDumbParent = $parent ['type'] == 'equation' && in_array ( $parent ['operator'], self::$arDumbOperators );
				} else {
					$bDumbParent = true;
				}
	...
	protected function createValue($tree, $bDumbParent) {
		...
		if (SparqlVariable::isVariable ( $strValue )) {
			if (! isset ( $this->sg->arUnionVarAssignments [$this->nUnionCount] [$strValue] )) {
				throw new SparqlEngineDb_SqlGeneratorException ( 'Unknown variable in filter: ' . $strValue );
			}
			...
			if ($bDumbParent) {
				...
				return self::mkVal ( '(CASE' . ' WHEN ' . $this->getDatatypeCol ( $tree ) . ' = "' . self::$typeXsdBoolean . '"' . ' THEN IF(LOWER(' . $this->getValueCol ( $tree ) . ') = "true", TRUE, FALSE)' . ' ELSE ' . $this->getValueCol ( $tree ) . ' END)', self::$typeVariable );
			} else {
				return self::mkVal ( $this->getValueCol ( $tree ), self::$typeVariable );
			}
		}
		
		if ($this->isNumber ( $tree )) {
			return $strValue;
		} else if ($tree ['quoted'] === false && (! isset ( $tree ['datatype'] ) || $tree ['datatype'] != self::$typeXsdBoolean)) {
			...
			if ($strValueNew === false) {
				if ($strValue [0] == '<' && substr ( $strValue, - 1 ) == '>') {
					...
					var_dump ( $tree );
					throw new SparqlEngineDb_SqlGeneratorException ( 'Unexpected value "' . $strValue . '" (expected datatype)' );
				}
			} else {
			...
			switch ($tree ['datatype']) {
				case self::$typeXsdBoolean :
					...
					return self::mkVal ( ( bool ) $strValue ? 'TRUE' : 'FALSE', self::$typeXsdBoolean );
				
				default :
					throw new SparqlEngineDb_SqlGeneratorException ( 'Unsupported datatype "' . $tree ['datatype'] );
			}
		}
		
		return self::mkVal ( '"' . addslashes ( $strValue ) . '"', self::$typeXsdString );
	...
	protected function createBooleanValue($tree) {
		...
		if (! $this->isObject ( $tree )) {
			...
			return self::mkVal ( $strTable . '.' . $strSqlCol . ' != ""', self::$typeXsdBoolean );
		}
		...
		return self::mkVal ( '(' . "(($cType = '' || $cType = '$xsd#string') AND $cValue != '')" . " OR ($cType = '$xsd#boolean' AND $cValue != 'false')" . " OR (($cType = '$xsd#integer' || $cType = '$xsd#double') AND CAST($cValue AS DECIMAL(15,10)) != 0)" . //plain check for all unknown datatypes
		" OR ($cType != '' AND $cType != '$xsd#string' AND $cType != '$xsd#boolean' AND $cType != '$xsd#integer' AND $cType != '$xsd#double' AND $cValue != '')" . ')', self::$typeXsdBoolean );
	} //protected function createBooleanValue($tree)
	...
	protected function createFunction($tree) {
		...
		if (! isset ( self::$arFuncParamNumbers [$strFuncName] )) {
			throw new SparqlEngineDb_SqlGeneratorException ( 'Unsupported FILTER function: ' . $strFuncName );
		}
		
		$nParams = count ( $tree ['parameter'] );
		if ($nParams < self::$arFuncParamNumbers [$strFuncName] [0] || $nParams > self::$arFuncParamNumbers [$strFuncName] [1]) {
			throw new SparqlEngineDb_SqlGeneratorException ( 'Wrong parameter count for FILTER function: ' . $strFuncName . ' (got ' . $nParams . ', expected ' . self::$arFuncParamNumbers [$strFuncName] [0] . '-' . self::$arFuncParamNumbers [$strFuncName] [1] );
		}
	...
	protected function createEquation($tree) {
		...
		if (($this->isNumber ( $tree ['operand1'] ) || $this->isPlainString ( $tree ['operand1'] )) && $this->isObject ( $tree ['operand2'] )) {
		...
		if ($this->isObject ( $tree ['operand1'] )) {
			...
			if (isset ( $tree ['operand2'] ['language'] )) {
				...
				$strExtra .= ' AND ' . $strColLanguage . ' = "' . addslashes ( $tree ['operand2'] ['language'] ) . '"';
			}
			
			if ($this->isNumber ( $tree ['operand2'] )) {
				$strColDatatype = $this->getDatatypeCol ( $tree ['operand1'] );
				$strExtra .= ' AND (' . $strColDatatype . '  = "' . self::$typeXsdDouble . '"' . ' OR ' . $strColDatatype . '  = "' . self::$typeXsdInteger . '"' . ')';
				return $strIsNull . '(' . 'CAST(' . $val1 . ' AS DECIMAL(15,10))' . ' ' . $tree ['operator'] . ' ' . $val2 . $strExtra . ')' . $strIsNullEnd;
			}
		}
		
		//I don't check the operator since it is already checked in the parser
		return $strIsNull . '(' . '(' . $val1 . ' ' . $tree ['operator'] . ' ' . $val2 . $strExtra . ')' . $this->createTypeEquation ( $tree ['operator'], $val1, $val2, $tree ['operand1'], $tree ['operand2'] ) . ')' . $strIsNullEnd;
	} //protected function createEquation($tree)
	...
	protected function createTypeEquation($operator, $val1, $val2, $tree1, $tree2) {
		...
		if ($so1 && $so2) {
			...
			if ($o1 && $o2) {
				...
				$sql .= ' AND ' . $this->getDatatypeCol ( $tree1 ) . ' = ' . $this->getDatatypeCol ( $tree2 );
			}
			return $sql;
		}
	...
	protected function createSingleTypeCheck($val, $tree) {
		if ($val->type == self::$typeXsdString) {
			...
			return ' AND (' . $this->getDatatypeCol ( $tree ) . ' = "' . $val->type . '"' . ' OR ' . $this->getDatatypeCol ( $tree ) . ' = ""' . ')';
		}
    ...
    */
	...
	protected function createFunction_bound($tree) {
		if ($this->isVariable ( $tree ['parameter'] [0] )) {
			return self::mkVal ( $this->getValueCol ( $tree ['parameter'] [0] ) . ' IS NOT NULL', self::$typeXsdBoolean );
		}
	...
	protected function createFunction_datatype($tree) {
		if (! $this->isObject ( $tree ['parameter'] [0] )) {
			throw new SparqlEngineDb_SqlGeneratorException ( 'datatype\'s first parameter needs to be an object' );
		}
		
		return self::mkVal ( $this->getDatatypeCol ( $tree ['parameter'] [0] ), self::$typeXsdString );
	} //protected function createFunction_datatype($tree)
	...
	protected function createFunction_isblank($tree) {
		if (! $this->isObjectOrSubject ( $tree ['parameter'] [0] )) {
			throw new SparqlEngineDb_SqlGeneratorException ( 'isBlank\'s first parameter needs to be an object or subject' );
		}
		
		return self::mkVal ( $this->getIsCol ( $tree ['parameter'] [0] ) . ' = "b"', self::$typeXsdBoolean );
	} //protected function createFunction_isblank($tree)
	...
	protected function createFunction_isliteral($tree) {
		...
		return self::mkVal ( $this->isPlainString ( $tree ) ? 'TRUE' : 'FALSE', self::$typeXsdBoolean );
	} //protected function createFunction_isliteral($tree)
	

	protected function createFunction_isuri($tree) {
		if ($this->isObjectOrSubject ( $tree ['parameter'] [0] )) {
			return self::mkVal ( $this->getIsCol ( $tree ['parameter'] [0] ) . ' = "r"', self::$typeXsdBoolean );
		} else {
			//predicates are iris
			return self::mkVal ( 'TRUE', self::$typeXsdBoolean );
		}
	...
	protected function createFunction_lang($tree) {
		if (! $this->isObject ( $tree ['parameter'] [0] )) {
			throw new SparqlEngineDb_SqlGeneratorException ( 'lang\'s first parameter needs to be an object' );
		}
		return self::mkVal ( $this->getLangCol ( $tree ['parameter'] [0] ), self::$typeXsdString );
	} //protected function createFunction_lang($tree)
	...
	protected function createFunction_langmatches($tree) {
		...
		if ($tree ['parameter'] [0] ['type'] != 'function' || $tree ['parameter'] [0] ['name'] != 'lang') {
			throw new SparqlEngineDb_SqlGeneratorException ( 'langMatches\' first parameter needs to be a lang() function' );
		}
		
		if (! $this->isPlainString ( $tree ['parameter'] [1] )) {
			throw new SparqlEngineDb_SqlGeneratorException ( 'langMatches\' second parameter needs to be a string' );
		}
		...
		switch ($lang) {
			...
			default :
				...
				// en -> en, en-US
				$sql = '(' . $col . ' = "' . addslashes ( $lang ) . '" OR ' . $col . ' LIKE "' . addslashes ( $lang ) . '-%")';
				break;
		}
	...
	protected function createFunction_regex($tree) {
		...
		if (isset ( $tree ['parameter'] [2] )) {
			...
			switch ($strMod) {
				...
				default :
					throw new SparqlEngineDb_SqlGeneratorException ( 'Unsupported regex modifier "' . $strMod . '"' );
			}
		} else {
			$sql = $strVar . ' REGEXP ' . $strRegex;
		}
	...
	protected function createFunction_sameterm($tree) {
		...
		return self::mkVal ( $this->createTreeSql ( $tree ['parameter'] [0], $tree ) . ' = ' . $this->createTreeSql ( $tree ['parameter'] [1], $tree ), self::$typeXsdBoolean );
	} //protected function createFunction_sameterm($tree)
	...
	protected function createFunction_str($tree) {
		if ($this->isObject ( $tree ['parameter'] [0] )) {
			return self::mkVal ( '(CASE ' . $this->getIsCol ( $tree ['parameter'] [0] ) . ' WHEN "b" THEN ""' . ' ELSE ' . $this->getValueCol ( $tree ['parameter'] [0] ) . ' END)', self::$typeXsdString );
		}
	...
	protected function createFunction_xsd_datetime($tree) {
		...
		if ($val->type == self::$typeXsdDateTime || $val->type == self::$typeVariable) {
			//no need to cast again
			return $val;
		}
		return self::mkVal ( $this->getDateConversionSql ( $val ), self::$typeXsdDateTime );
	} //protected function createFunction_xsd_datetime($tree)
    ...
    */
	...
	protected function isNumber($tree) {
		return $tree ['type'] == 'value' && $tree ['quoted'] === false && preg_match ( '#^[0-9.]+$#', $tree ['value'] ) && floatval ( $tree ['value'] ) == $tree ['value'];
	} //protected function isNumber($tree)
	...
	protected function isObject($tree) {
		return $this->isVariable ( $tree ) && $this->sg->arUnionVarAssignments [$this->nUnionCount] [$tree ['value']] [1] == 'o';
	...
	protected function isObjectOrSubject($tree) {
		return $this->isVariable ( $tree ) && ($this->sg->arUnionVarAssignments [$this->nUnionCount] [$tree ['value']] [1] == 'o' || $this->sg->arUnionVarAssignments [$this->nUnionCount] [$tree ['value']] [1] == 's');
	} //protected function isObjectOrSubject($tree)
	...
	protected function isPlainString($tree) {
		return $tree ['type'] == 'value' && $tree ['quoted'] === true;
	} //protected function isPlainString($tree)
	

	protected function isValueButNotVariableNorString($tree) {
		return $tree ['type'] == 'value' && $tree ['type'] ['quoted'] === false && ! SparqlVariable::isVariable ( $tree ['value'] );
	} //protected function isValueButNotVariableNorString($tree)
