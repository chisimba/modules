...
class SparqlEngineDb_TypeSorter {
	 ...
	 *   @var array
	 */
	public static $arTypeNumbers = array (null => 0, //empty
'b' => 1, //blank
'r' => 2, //resource
'l' => 3 )//literal
;
	 ...
	 *   @var array
	 */
	public static $arCastTypes = array ('http://www.w3.org/2001/XMLSchema#integer' => 'SIGNED INTEGER', 'http://www.w3.org/2001/XMLSchema#decimal' => 'DECIMAL', //yes, this does not work with multiple time zones.
	'http://www.w3.org/2001/XMLSchema#dateTime' => 'CHAR', 'http://www.w3.org/2001/XMLSchema#date' => 'CHAR' );
	...
	public function getOrderifiedSqls($arSqls) {
		...
		$arTypeSets = $this->orderTypeSets ( $this->getTypeSets ( $arSpecial, $strFrom, $strWhere ) );
	...
	public function willBeDataDependent() {
		$strResultForm = $this->query->getResultForm ();
		return ! ($strResultForm == 'ask' || $strResultForm == 'count' || count ( $this->getSpecialOrderVariables () ) == 0);
	} //public function willBeDataDependent()
	...
	protected function isSpecialOrderVariable($strVar) {
		return ! isset ( $this->arUsedVarTypes [$strVar] ['s'] ) && ! isset ( $this->arUsedVarTypes [$strVar] ['p'] );
	...
	protected function getTypeSets($arSpecialVars, $strFrom, $strWhere) {
		...
		foreach ( $arSpecialVars as $strSparqlVar ) {
			if ($this->arVarAssignments [$strSparqlVar] [1] == 'o') {
				$arSel [] = $this->arVarAssignments [$strSparqlVar] [0] . '.l_datatype' . ' as "' . $strSparqlVar . '-datatype"';
			}
			$strTypeCol = $this->arVarAssignments [$strSparqlVar] [1] == 'o' ? 'object_is' : 'subject_is';
			$this->arVarAssignments [$strSparqlVar] [2] = $strTypeCol;
			$arSel [] = $this->arVarAssignments [$strSparqlVar] [0] . '.' . $strTypeCol . ' as "' . $strSparqlVar . '-type"';
		}
		...
		if ($arResult === false) {
			throw new SparqlEngineDb_SqlGeneratorException ( 'Error reading typesets: ' . $sql );
		}
	...
	protected function orderTypeSets($arTypes) {
		...
		if ($arSM ['order by'] !== null) {
			...
			foreach ( $arSM ['order by'] as $arVar ) {
				$this->arDirection [$arVar ['val']] = (strtoupper ( $arVar ['type'] ) == 'ASC' ? 1 : - 1);
			}
		}
	...
	public function compareTypeSet($arTs1, $arTs2) {
		foreach ( $arTs1 as $strSparqlVar => $arType1 ) {
			//compare types
			$n = self::$arTypeNumbers [$arType1 ['type']] - self::$arTypeNumbers [$arTs2 [$strSparqlVar] ['type']];
			if ($n != 0) {
			...
			$n = self::compareXsdType ( $arType1 ['datatype'], $arTs2 [$strSparqlVar] ['datatype'] );
		...
		foreach ( $arSM ['order by'] as $arVar ) {
			...
			if (isset ( $this->arUsedVarAssignments [$strSparqlVar] )) {
				if (! isset ( $arTypeSet [$strSparqlVar] ['datatype'] ) || $arTypeSet [$strSparqlVar] ['datatype'] == '') {
					...
					$sqlOrder [] = self::getCastMethod ( $arTypeSet [$strSparqlVar] ['datatype'], $this->arUsedVarAssignments [$strSparqlVar] ) . ' ' . strtoupper ( $arVar ['type'] );
				}
			}
		}
	...
	protected function getTypesetWhereClause($arTypeSet) {
		...
		foreach ( $arTypeSet as $strSparqlVar => $arType ) {
			//check type
			$strWhereTypes .= ' AND ' . $this->arVarAssignments [$strSparqlVar] [0] . '.' . $this->arVarAssignments [$strSparqlVar] [2] . $this->getStringNullComparison ( $arType ['type'] );
			//check datatype
			if (isset ( $arType ['datatype'] )) {
				$strWhereTypes .= ' AND ' . $this->arVarAssignments [$strSparqlVar] [0] . '.l_datatype ' . $this->getStringNullComparison ( $arType ['datatype'] );
			}
		}
