...
class SparqlEngineDb_SqlGenerator {
	 ...
	 *   @var array
	 */
	public static $arTableColumnNames = array ('s' => array ('value' => 'subject', 'is' => 'subject_is' ), 'p' => array ('value' => 'predicate' ), 'o' => array ('value' => 'object', 'is' => 'object_is' ) );
	...
	function createSql() {
		...
		foreach ( $this->query->getResultPart () as $graphPattern ) {
			...
			if ($arTriplePattern != null) {
				foreach ( $arTriplePattern as $triplePattern ) {
					list ( $arSelect [$this->nUnionCount] [], $arFrom [$this->nUnionCount] [], $arWhere [$this->nUnionCount] [] ) = $this->getTripleSql ( $triplePattern, $graphPattern, $arResultVars );
					++ $this->nTableId;
		...
		foreach ( $this->query->getResultPart () as $graphPattern ) {
			...
			if ($arConstraints != null) {
				foreach ( $arConstraints as $constraint ) {
					$arWhere [$this->nUnionCount] [count ( $arWhere [$this->nUnionCount] ) - 1] .= $filterGen->createFilterSql ( $constraint->getTree (), $graphPattern->getOptional () !== null, $this->nUnionCount );
				}
			}
			++ $this->nGraphPatternCount;
		}
		...
		switch ($strResultForm) {
			...
			case 'select distinct' :
				...
				foreach ( $arSelect as $nUnionCount => $arSelectPart ) {
					...
					if (count ( $arSelectPart ) == 0 || (count ( $arSelectPart ) == 1 && $arSelectPart [0] == '')) {
		...
		foreach ( $arStrSelect as $nUnionCount => $arSelectPart ) {
			$arSqls [] = array ('select' => $arStrSelect [$nUnionCount], 'from' => ' FROM ' . implode ( ' ', self::removeNull ( $arFrom [$nUnionCount] ) ), 'where' => ' WHERE ' . self::fixWhere ( implode ( ' ', self::removeNull ( $arWhere [$nUnionCount] ) ) ) );
		}
		return $arSqls;
	} //function createSql()
	...
	protected function createVariableSelectArray($chType, $varname, $strTablePrefix) {
		...
		if ($var !== false) {
			...
			if ($func != null) {
				if ($func == 'datatype') {
					if ($chType != 'o') {
						throw new SparqlEngineDb_SqlGeneratorException ( 'datatype() works on objects only' );
					}
					return array ($strTablePrefix . '.l_datatype as "' . $strTablePrefix . '.' . $this->getSqlVariableNameValue ( $var ) . '"', '"r"' . ' as "' . $strTablePrefix . '.' . $this->getSqlVariableNameIs ( $var ) . '"', '""' . ' as "' . $strTablePrefix . '.' . $this->getSqlVariableNameLanguage ( $var ) . '"', '""' . ' as "' . $strTablePrefix . '.' . $this->getSqlVariableNameDatatype ( $var ) . '"' );
				} else if ($func == 'lang') {
					if ($chType != 'o') {
						throw new SparqlEngineDb_SqlGeneratorException ( 'lang() works on objects only' );
					}
					return array ($strTablePrefix . '.l_language as "' . $strTablePrefix . '.' . $this->getSqlVariableNameValue ( $var ) . '"', '"l"' . ' as "' . $strTablePrefix . '.' . $this->getSqlVariableNameIs ( $var ) . '"', '""' . ' as "' . $strTablePrefix . '.' . $this->getSqlVariableNameLanguage ( $var ) . '"', '""' . ' as "' . $strTablePrefix . '.' . $this->getSqlVariableNameDatatype ( $var ) . '"' );
				} else {
					throw new SparqlEngineDb_SqlGeneratorException ( 'Unsupported function for select "' . $func . '"' );
				}
			}
		}
		...
		switch ($chType) {
			case 's' :
				return array ($strTablePrefix . '.subject as "' . $strTablePrefix . '.' . $this->getSqlVariableNameValue ( $varname ) . '"', $strTablePrefix . '.subject_is as "' . $strTablePrefix . '.' . $this->getSqlVariableNameIs ( $varname ) . '"' );
			case 'p' :
				return array ($strTablePrefix . '.predicate as "' . $strTablePrefix . '.' . $this->getSqlVariableNameValue ( $varname ) . '"' );
			case 'o' :
				return array ($strTablePrefix . '.object as "' . $strTablePrefix . '.' . $this->getSqlVariableNameValue ( $varname ) . '"', $strTablePrefix . '.object_is as "' . $strTablePrefix . '.' . $this->getSqlVariableNameIs ( $varname ) . '"', $strTablePrefix . '.l_language as "' . $strTablePrefix . '.' . $this->getSqlVariableNameLanguage ( $varname ) . '"', $strTablePrefix . '.l_datatype as "' . $strTablePrefix . '.' . $this->getSqlVariableNameDatatype ( $varname ) . '"' );
			default :
				throw new SparqlEngineDb_SqlGeneratorException ( 'Unknown sentence type "' . $chType . "', one of (s,p,o) expected" );
		}
		...
		foreach ( $arSelect as $nUnionCount => $arUnionVars ) {
			...
			foreach ( $arVars as $strVar ) {
				...
				if ($nCount == 0) {
					...
					$ar [] = 'NULL as ' . '"' . $this->arVarAssignments [$strVar] [0] . '.' . $this->arVarAssignments [$strVar] ['sql_value'] . '"';
				}
				if ((isset ( $this->arUsedVarTypes [$strVar] ['o'] ) || isset ( $this->arUsedVarTypes [$strVar] ['s'] )) && $nCount < 2) {
					//it's a subject or object, but we don't want the type
					$ar [] = 'NULL as ' . '"' . $this->arVarAssignments [$strVar] [0] . '.' . $this->arVarAssignments [$strVar] ['sql_is'] . '"';
				}
				if (isset ( $this->arUsedVarTypes [$strVar] ['o'] ) && $nCount < 4) {
					...
					$ar [] = 'NULL as ' . '"' . $this->arVarAssignments [$strVar] [0] . '.' . $strColLanguage . '"';
					$ar [] = 'NULL as ' . '"' . $this->arVarAssignments [$strVar] [0] . '.' . $strColDatatype . '"';
				}
				$ars [] = implode ( ', ', $ar );
			}
	...
	protected static function getSqlEqualityCondition($arNew, $arOld) {
		...
		if ($chTypeNew == 'p' || $chTypeOld == 'p') {
			...
			return $strTablePrefixNew . '.' . self::$arTableColumnNames [$chTypeNew] ['value'] . ' = ' . $strTablePrefixOld . '.' . self::$arTableColumnNames [$chTypeOld] ['value'];
		} else if ($chTypeNew == 's' || $chTypeOld == 's') {
			//check value and type
			return $strTablePrefixNew . '.' . self::$arTableColumnNames [$chTypeNew] ['value'] . ' = ' . $strTablePrefixOld . '.' . self::$arTableColumnNames [$chTypeOld] ['value'] . ' AND ' . $strTablePrefixNew . '.' . self::$arTableColumnNames [$chTypeNew] ['is'] . ' = ' . $strTablePrefixOld . '.' . self::$arTableColumnNames [$chTypeOld] ['is'];
		} else {
			//two objects -> check everything
			return $strTablePrefixNew . '.object = ' . $strTablePrefixOld . '.object' . ' AND ' . $strTablePrefixNew . '.object_is = ' . $strTablePrefixOld . '.object_is' . ' AND ' . $strTablePrefixNew . '.l_language = ' . $strTablePrefixOld . '.l_language' . ' AND ' . $strTablePrefixNew . '.l_datatype = ' . $strTablePrefixOld . '.l_datatype';
		}
	} //protected static function getSqlEqualityCondition($arNew, $arOld)
	...
	protected static function isResultVar($strVar, &$arResultVars) {
		foreach ( $arResultVars as $var ) {
			if ($var == '*') {
				return true;
			} else if ((is_string ( $var ) && $var == $strVar) || (is_object ( $var ) && $var->getVariable () == $strVar)) {
				return true;
			}
		}
		return false;
	...
	public static function isPreparedVariable($bject) {
		return is_string ( $bject ) && strlen ( $bject ) >= 3 && ($bject [0] == '?' || $bject [0] == '$') && ($bject [1] == '?' || $bject [1] == '$');
	} //public static function isPreparedVariable($bject)
	...
	protected function getPreparedVariablePlaceholder($strVariable) {
		...
		if (! isset ( $this->arPlaceholders [$strName] )) {
			$this->arPlaceholders [$strName] = '@$%_PLACEHOLDER_' . count ( $this->arPlaceholders ) . '_%$@';
		}
	...
	protected function getSqlVariableNameValue($var) {
		$strSparqlVar = ( string ) $var;
		$this->arVarAssignments [$strSparqlVar] ['sql_value'] = 'value_' . $this->getSqlVariableName ( $var );
	...
	protected function getSqlVariableNameIs($var) {
		$strSparqlVar = ( string ) $var;
		$this->arVarAssignments [$strSparqlVar] ['sql_is'] = 'is_' . $this->getSqlVariableName ( $var );
	...
	protected function getSqlVariableNameLanguage($var) {
		$strSparqlVar = ( string ) $var;
		$this->arVarAssignments [$strSparqlVar] ['sql_lang'] = 'lang_' . $this->getSqlVariableName ( $var );
	...
	protected function getSqlVariableNameDatatype($var) {
		$strSparqlVar = ( string ) $var;
		$this->arVarAssignments [$strSparqlVar] ['sql_type'] = 'type_' . $this->getSqlVariableName ( $var );
