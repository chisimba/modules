	...
	public static function convertFromResult($arVartable, SparqlEngine $engine, $resultform = false) {
		if (is_object ( $resultform )) {
			if ($resultform instanceof SparqlEngine_ResultRenderer) {
				return $resultform->convertFromResult ( $arVartable, $engine->getQuery (), $engine );
			} else {
				throw new Exception ( 'Result renderer object needs to implement' . ' SparqlEngine_ResultRenderer interface' );
			}
		}
		...
		if ($strClass = self::loadClass ( $resultform )) {
			...
			if ($rrObj instanceof SparqlEngine_ResultRenderer) {
				return $rrObj->convertFromResult ( $arVartable, $engine->getQuery (), $engine );
			} else {
				throw new Exception ( 'Result renderer class "' . $strClass . '" needs to implement' . ' SparqlEngine_ResultRenderer interface' );
			}
		} else {
