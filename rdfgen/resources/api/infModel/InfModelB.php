...
class InfModelB extends InfModel {
	...
	function _infFind($subject, $predicate, $object, $searchStringIndex, $findOnlyFirstMatching = false, $offset = 0, $resultCount = 0) {
		...
		//is false
		if ($predicate == null || (is_a ( $predicate, 'Node' ) && ! in_array ( $predicate->getLabel (), $this->supportedInference )))
			...
			foreach ( $this->_findRuleEntailmentInIndex ( $subject, $predicate, $object ) as $ruleKey ) {
				...
				if (! in_array ( $serializedRuleStatement, $searchStringIndex ) && ! in_array ( $serializedRuleStatement, $this->findDeadEnds )) {
						...
						if (isset ( $infFindResult [0] )) {
							foreach ( $infFindResult as $statement ) {
								...
								if (! in_array ( $newStatement->getLabelPredicate (), $this->supportedInference ))
									...
									if ($this->_nodeEqualsFind ( $subject, $newStatement->getSubject () ) && $this->_nodeEqualsFind ( $predicate, $newStatement->getPredicate () ) && $this->_nodeEqualsFind ( $object, $newStatement->getObject () )) {
										//Add to results
										$return [] = $newStatement;
										$resultCount ++;
