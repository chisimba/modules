...
class N3Serializer extends Object {
	...
	function &serialize(&$m) {
		...
		if (! HIDE_ADVERTISE) {
			$this->res .= '# Generated by N3Serializer.php from RDF RAP.' . LINEFEED . '# http://www.wiwiss.fu-berlin.de/suhl/bizer/rdfapi/index.html' . LINEFEED . LINEFEED;
		}
	...
	function doResource(&$r, $bEmbedded = false, $strIndent = '    ') {
		...
		if (is_a ( $r, 'Resource' )) {
			if (is_a ( $r, 'BlankNode' )) {
				...
				$compress = (N3SER_BNODE_SHORT || $this->styleCompress) && (count ( $rbn->triples ) == 0 || (count ( $rbn->triples ) == 1 && $bEmbedded));
				if ($compress) {
					$out .= '[';
				} else {
					$out .= '_:' . $r->getLabel ();
		...
		foreach ( $ts->triples as $t ) {
			...
			if (is_a ( $o, 'Resource' )) {
				...
				if (is_a ( $o, 'BlankNode' )) {
					if ($this->styleNest && $this->styleCompress && ! isset ( $this->done [$o->getURI ()] )) {
