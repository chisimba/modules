...
class N3Parser extends Object {
	...
	function N3Parser() {
		...
		//	    $Univar, 'a', '{', '}', '\(', '\)', '\[', '\]', ',', ';', '\.', $WS, $Comment);
		$t = array ($Datatype_URI, $Datatype, $LLiteral, $URI, $Literal, $PrefixDecl, $QName, $Number, $Boolean, $bNode, $Prefix, $Univar, 'a', '=', '{', '}', '\(', '\)', '\[', '\]', ',', ';', '\.', $WS, $Comment, $LangTag );
	...
	function applyStuff($prefixes, $list) {
		...
		for($i = 0; $i < count ( $list ); $i ++) {
			...
			} else if ((! strstr ( '<_"\'?.;,{}[]()@', $list [$i] {0} )) && (substr ( $list [$i], 0, 3 ) != '^^<')) {
				//prefix or unknown
				$_r = explode ( ':', $list [$i] );
				$ns = $_r [0] . ':';
				$name = $_r [1];
	...
	function statementize($list) {
		...
		if (count ( $list ) == 3)
			return array ($list );
		if (count ( $list ) < 3) {
			throw new Exception ( 'N3 statement too short,' . ' only ' . count ( $list ) . ' elements instead of 3:' . "\n" . implode ( "\n", $list ) );
		}
