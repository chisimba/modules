...
class RdfParser extends Object {
	...
	function _is_rdf_property_attribute_literal($local_name) {
		return ($local_name == RDF_VALUE) || ($local_name == RDF_BAG) || ($local_name == RDF_SEQ) || ($local_name == RDF_ALT) || ($local_name == RDF_STATEMENT) || ($local_name == RDF_PROPERTY) || ($local_name == RDF_LIST);
	}
	...
	function _is_forbidden_rdf_property_attribute($local_name) {
		return ($local_name == RDF_RDF) || ($local_name == RDF_DESCRIPTION) || ($local_name == RDF_ID) || ($local_name == RDF_ABOUT) || ($local_name == RDF_BAG_ID) || ($local_name == RDF_PARSE_TYPE) || ($local_name == RDF_RESOURCE) || ($local_name == RDF_NODEID) || ($local_name == RDF_LI) || ($local_name == RDF_ABOUT_EACH) || ($local_name == RDF_ABOUT_EACH_PREFIX) || ($local_name == RDF_DATATYPE);
	}
	...
	function _is_rdf_property_element($local_name) {
		return ($local_name == RDF_TYPE) || ($local_name == RDF_SUBJECT) || ($local_name == RDF_PREDICATE) || ($local_name == RDF_OBJECT) || ($local_name == RDF_VALUE) || ($local_name == RDF_LI) || ($local_name == RDF_SEEALSO) || ($local_name == RDF_BAG) || ($local_name == RDF_SEQ) || ($local_name == RDF_ALT) || ($local_name == RDF_STATEMENT) || ($local_name == RDF_PROPERTY) || ($local_name == RDF_LIST) || ($local_name == RDF_FIRST) || ($local_name == RDF_REST) || ($local_name {0} == '_');
	}
	...
	function _is_forbidden_rdf_property_element($local_name) {
		return ($local_name == RDF_RDF) || ($local_name == RDF_DESCRIPTION) || ($local_name == RDF_ID) || ($local_name == RDF_ABOUT) || ($local_name == RDF_BAG_ID) || ($local_name == RDF_PARSE_TYPE) || ($local_name == RDF_RESOURCE) || ($local_name == RDF_NODEID) || ($local_name == RDF_ABOUT_EACH) || ($local_name == RDF_ABOUT_EACH_PREFIX) || ($local_name == RDF_DATATYPE);
	}
	...
	function _is_forbidden_rdf_node_element($local_name) {
		return ($local_name == RDF_RDF) || ($local_name == RDF_ID) || ($local_name == RDF_ABOUT) || ($local_name == RDF_BAG_ID) || ($local_name == RDF_PARSE_TYPE) || ($local_name == RDF_RESOURCE) || ($local_name == RDF_NODEID) || ($local_name == RDF_LI) || ($local_name == RDF_ABOUT_EACH) || ($local_name == RDF_ABOUT_EACH_PREFIX) || ($local_name == RDF_DATATYPE);
	}
...
*/
				...
				if ($reference_path != '') {
					if ($reference_path {0} == '/' || $reference_path {0} == "\\") {
						if ($reference_path {1} == '/' || $reference_path {1} == "\\") {
							...
							$result_path = $reference_path;
					} elseif (substr ( $reference_path, 0, 3 ) == '../' || substr ( $reference_path, 0, 3 ) == '..\\') {
						$slash = $reference_path {2};
						while ( $base_path != '' && (substr ( $reference_path, 0, 3 ) == '../' || substr ( $reference_path, 0, 3 ) == '..\\') ) {
		...
		if ($id) {
			if ($this->_istalpha ( $id {0} ) || $id {0} == '_') {
				...
				while ( $result != FALSE && ++ $i < $len ) {
					if (! ($this->_istalnum ( $id {$i} ) || $id {$i} == '.' || $id {$i} == '-' || $id {$i} == '_')) {
						$result = FALSE;
					}
				}
			}
	...
	function _report_statement($subject_type, $subject, $predicate, $ordinal, $object_type, $object, $xml_lang, $bag_id, $statements, $statement_id, $datatype) {
		...
		if ($bag_id) {
			...
			$this->_report_statement ( RDF_SUBJECT_TYPE_URI, $bag_id, $predicate_buffer, $statements, RDF_OBJECT_TYPE_BNODE, $statement_id, '', '', '', '', $datatype );
		}
		
		if ($statement_id) {
			// rdf:type = rdf:Statement
			$this->_report_statement ( $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_TYPE, 0, RDF_OBJECT_TYPE_RESOURCE, RDF_NAMESPACE_URI . RDF_STATEMENT, '', '', '', '', $datatype );
			...
			// rdf:subject
			$this->_report_statement ( $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_SUBJECT, 0, $obj_type, $subject, '', '', '', '', $datatype );
			
			// rdf:predicate
			$this->_report_statement ( $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_PREDICATE, 0, RDF_OBJECT_TYPE_RESOURCE, $predicate, '', '', '', '', $datatype );
			
			// rdf:object
			$this->_report_statement ( $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_OBJECT, 0, $object_type, $object, '', '', '', '', $datatype );
		}
	}
	/**
	 * @param string $subject_type
	...
	function _handle_property_attributes($subject_type, $subject, $attributes, $xml_lang, $bag_id, $statements) {
		...
		for($i = 0; isset ( $attributes [$i] ); $i += 2) {
			$this->_split_name ( $attributes [$i], $attribute, $attribute_namespace_uri, $attribute_local_name );
			...
			if (RDF_NAMESPACE_URI == $attribute_namespace_uri) {
				if ($this->_is_rdf_property_attribute_literal ( $attribute_local_name )) {
					$this->_report_statement ( $subject_type, $subject, $predicate, 0, RDF_OBJECT_TYPE_LITERAL, $attribute_value, $xml_lang, $bag_id, $statements, '', '' );
				} else if ($this->_is_rdf_property_attribute_resource ( $attribute_local_name )) {
					$this->_report_statement ( $subject_type, $subject, $predicate, 0, RDF_OBJECT_TYPE_RESOURCE, $attribute_value, '', $bag_id, $statements, '', '' );
				} else if (($ordinal = $this->_is_rdf_ordinal ( $attribute_local_name )) != 0) {
					$this->_report_statement ( $subject_type, $subject, $predicate, $ordinal, RDF_OBJECT_TYPE_LITERAL, $attribute_value, $xml_lang, $bag_id, $statements, '', '' );
				} else if (($attribute_local_name != RDF_ABOUT) && ($attribute_local_name != RDF_RDF) && ($attribute_local_name != RDF_DESCRIPTION) && ($attribute_local_name != RDF_ID) && ($attribute_local_name != RDF_ABOUT_EACH) && ($attribute_local_name != RDF_ABOUT_EACH_PREFIX) && ($attribute_local_name != RDF_BAG_ID) && ($attribute_local_name != RDF_RESOURCE) && ($attribute_local_name != RDF_PARSE_TYPE) && ($attribute_local_name != RDF_PARSE_TYPE_LITERAL) && ($attribute_local_name != RDF_PARSE_TYPE_RESOURCE) && ($attribute_local_name != RDF_LI) && ($attribute_local_name != RDF_SUBJECT) && ($attribute_local_name != RDF_PREDICATE) && ($attribute_local_name != RDF_OBJECT) && ($attribute_local_name != RDF_NODEID) && ($attribute_local_name != RDF_DATATYPE) && ($attribute_local_name != RDF_SEEALSO) && ($attribute_local_name != RDF_NIL) && ($attribute_local_name != RDF_REST) && ($attribute_local_name != RDF_FIRST)) {
					$this->_report_statement ( $subject_type, $subject, $predicate, 0, RDF_OBJECT_TYPE_LITERAL, $attribute_value, $xml_lang, $bag_id, $statements, '', '' );
				}
	...
	function _handle_resource_element($namespace_uri, $local_name, $attributes, $parent) {
		...
		if (($local_name != RDF_DESCRIPTION) || ($namespace_uri != RDF_NAMESPACE_URI)) {
			...
			$this->_report_statement ( $this->rdf_parser ['top'] ['subject_type'], $this->rdf_parser ['top'] ['subject'], RDF_NAMESPACE_URI . RDF_TYPE, 0, RDF_OBJECT_TYPE_RESOURCE, $type, '', $this->rdf_parser ['top'] ['bag_id'], $this->rdf_parser ['top'] ['statements'], '', $datatype );
		...
		if ($parent) {
			...
			$this->_report_statement ( $parent ['parent'] ['subject_type'], $parent ['parent'] ['subject'], $parent ['predicate'], $parent ['ordinal'], $objtype, $this->rdf_parser ['top'] ['subject'], '', $parent ['parent'] ['bag_id'], $parent ['parent'] ['statements'], $parent ['statement_id'], $parent ['datatype'] );
		
		}
		
		if ($this->rdf_parser ['top'] ['has_property_attributes']) {
			$this->_handle_property_attributes ( $this->rdf_parser ['top'] ['subject_type'], $this->rdf_parser ['top'] ['subject'], $attributes, $this->rdf_parser ['top'] ['xml_lang'], $this->rdf_parser ['top'] ['bag_id'], $this->rdf_parser ['top'] ['statements'] );
		}
	}
	...
	function _handle_property_element(&$namespace_uri, &$local_name, &$attributes) {
		...
		for($i = 0; isset ( $attributes [$i] ); $i += 2) {
			$this->_split_name ( $attributes [$i], $buffer, $attribute_namespace_uri, $attribute_local_name );
		...
		if ($node_id) {
			...
			if ($statement_id) {
				// reify statement
				$this->_report_statement ( $this->rdf_parser ['top'] ['parent'] ['subject_type'], $this->rdf_parser ['top'] ['parent'] ['subject'], $this->rdf_parser ['top'] ['predicate'], $this->rdf_parser ['top'] ['ordinal'], RDF_OBJECT_TYPE_BNODE, $node_id, '', $this->rdf_parser ['top'] ['parent'] ['bag_id'], $this->rdf_parser ['top'] ['parent'] ['statements'], $this->rdf_parser ['top'] ['statement_id'], '' );
				$statement_id = '';
			} else {
				$this->_report_statement ( $this->rdf_parser ['top'] ['parent'] ['subject_type'], $this->rdf_parser ['top'] ['parent'] ['subject'], $this->rdf_parser ['top'] ['predicate'], $this->rdf_parser ['top'] ['ordinal'], RDF_OBJECT_TYPE_BNODE, $node_id, '', $this->rdf_parser ['top'] ['parent'] ['bag_id'], $this->rdf_parser ['top'] ['parent'] ['statements'], '', $datatype );
			}
		...
		if ($parse_type) {
			...
			if ($attribute_value == RDF_PARSE_TYPE_RESOURCE) {
				...
				$this->_report_statement ( $this->rdf_parser ['top'] ['parent'] ['subject_type'], $this->rdf_parser ['top'] ['parent'] ['subject'], $this->rdf_parser ['top'] ['predicate'], 0, RDF_OBJECT_TYPE_BNODE, $buffer, '', $this->rdf_parser ['top'] ['parent'] ['bag_id'], $this->rdf_parser ['top'] ['parent'] ['statements'], $this->rdf_parser ['top'] ['statement_id'], $datatype );
				...
				$this->_report_statement ( $this->rdf_parser ['top'] ['parent'] ['subject_type'], $this->rdf_parser ['top'] ['parent'] ['subject'], $this->rdf_parser ['top'] ['predicate'], 0, RDF_OBJECT_TYPE_XML, '', '', $this->rdf_parser ['top'] ['parent'] ['bag_id'], $this->rdf_parser ['top'] ['parent'] ['statements'], $this->rdf_parser ['top'] ['statement_id'], $datatype );
			...
			// since we are sure that this is now a resource property we can report it.
			$this->_report_statement ( $this->rdf_parser ['top'] ['parent'] ['subject_type'], $this->rdf_parser ['top'] ['parent'] ['subject'], $this->rdf_parser ['top'] ['predicate'], $this->rdf_parser ['top'] ['ordinal'], $object_type, $buffer, '', $this->rdf_parser ['top'] ['parent'] ['bag_id'], $this->rdf_parser ['top'] ['parent'] ['statements'], $this->rdf_parser ['top'] ['statement_id'], $datatype ); // should we allow IDs?
			...
			if ($this->rdf_parser ['top'] ['has_property_attributes']) {
				$this->_handle_property_attributes ( $subject_type, $buffer, $attributes, $this->rdf_parser ['top'] ['xml_lang'], $this->rdf_parser ['top'] ['bag_id'], $this->rdf_parser ['top'] ['statements'] );
			}
		}
	}
...
*/
		...
		for($i = 0; isset ( $attributes [$i] ); $i += 2) {
			...
			if ($attribute_namespace_uri == '' || $attribute_namespace_uri == RDF_NAMESPACE_URI) {
				...
				if ($attribute_local_name == RDF_ABOUT || $attribute_local_name == RDF_RESOURCE) {
					$this->rdf_parser ['top'] ['parent'] ['collection'] ['object_type'] [] = RDF_OBJECT_TYPE_RESOURCE;
				} elseif ($attribute_local_name == RDF_NODEID) {
					$this->rdf_parser ['top'] ['parent'] ['collection'] ['object_type'] [] = RDF_OBJECT_TYPE_BNODE;
				...
				if (! (($namespace_uri == RDF_NAMESPACE_URI || $namespace_uri == '') && ($local_name == RDF_DESCRIPTION || $local_name == RDF_LI))) {
					$this->_report_statement ( $tmp_subject_type, $id_buffer, RDF_NAMESPACE_URI . RDF_TYPE, '', RDF_OBJECT_TYPE_RESOURCE, $namespace_uri . $local_name, '', '', '', '', '' );
				}
			}
		}
	...
	function _handle_xml_end_element($name) {
		...
		$this->_split_name ( $name, $buffer, $namespace_uri, $local_name );
		
		$element = '</';
		
		if ($namespace_uri && isset ( $this->rdf_parser ['default_namespace'] ) && $namespace_uri != $this->rdf_parser ['default_namespace']) {
	...
	function _join_name_and_declare_prefix($namespace_uri, $local_name) {
		...
		if ($namespace_uri) {
			if (isset ( $this->rdf_parser ['default_namespace'] ) && $namespace_uri == $this->rdf_parser ['default_namespace']) {
				$name .= $local_name;
				
				if (! isset ( $this->rdf_parser ['xml_literal'] ['declared_ns'] ['_DEFAULT_'] ) && $namespace_uri != XML_NAMESPACE_URI) {
					$name .= ' xmlns=' . '\"' . $namespace_uri . '\"';
					
					$this->rdf_parser ['xml_literal'] ['declared_ns'] ['_DEFAULT_'] = $this->rdf_parser ['xml_literal'] ['depth'];
				}
			} else {
				$ns_prefix = $this->rdf_parser ['namespaces'] [$namespace_uri];
				$name .= $ns_prefix . ':' . $local_name;
				
				if (! isset ( $this->rdf_parser ['xml_literal'] ['declared_ns'] [$ns_prefix] ) && $namespace_uri != XML_NAMESPACE_URI) {
					$name .= " xmlns:$ns_prefix=" . '\"' . $namespace_uri . '\"';
					
					$this->rdf_parser ['xml_literal'] ['declared_ns'] [$ns_prefix] = $this->rdf_parser ['xml_literal'] ['depth'];
				}
			}
	...
	function _end_collection() {
		...
		if (isset ( $this->rdf_parser ['top'] ['collection'] )) {
			...
			for($i = 0; isset ( $this->rdf_parser ['top'] ['collection'] ['object_label'] [$i] ); $i ++) {
				...
				$this->_report_statement ( RDF_SUBJECT_TYPE_BNODE, $subject, RDF_NAMESPACE_URI . RDF_FIRST, '', $this->rdf_parser ['top'] ['collection'] ['object_type'] [$i], $this->rdf_parser ['top'] ['collection'] ['object_label'] [$i], '', '', '', '', '' );
				...
				$this->_report_statement ( RDF_SUBJECT_TYPE_BNODE, $subject, RDF_NAMESPACE_URI . RDF_REST, '', $obj_type_2, $object_2, '', '', '', '', '' );
	...
	function _start_element_handler($parser, $name, $attributes) {
        ...
        $this->rdf_parser ['top'] ['data'] = '';
				...
				$this->rdf_parser ['top'] ['state'] = IN_DESCRIPTION;
				$this->_handle_resource_element ( $namespace_uri, $local_name, $attributes, $this->rdf_parser ['top'] ['parent'] );
				break;
	...
	function _end_literal_property() {
		...
		$this->_report_statement ( $this->rdf_parser ['top'] ['parent'] ['subject_type'], $this->rdf_parser ['top'] ['parent'] ['subject'], $this->rdf_parser ['top'] ['predicate'], $this->rdf_parser ['top'] ['ordinal'], RDF_OBJECT_TYPE_LITERAL, $this->rdf_parser ['top'] ['data'], $this->rdf_parser ['top'] ['xml_lang'], $this->rdf_parser ['top'] ['parent'] ['bag_id'], $this->rdf_parser ['top'] ['parent'] ['statements'], $this->rdf_parser ['top'] ['statement_id'], $this->rdf_parser ['top'] ['datatype'] );
