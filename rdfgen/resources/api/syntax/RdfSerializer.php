...
class RdfSerializer extends Object {
	...
	function &serialize(&$model, $xml_default_namespace = NULL, $encoding = DEFAULT_ENCODING) {
		...
		if ($this->sort_model) {
			...
			foreach ( $model->triples as $key => $statement ) {
				$stmkey = $statement->subj->getURI () . $statement->pred->getURI () . (is_a ( $statement->obj, 'Literal' ) ? '"' . $statement->obj->getLabel () . '"@' . $statement->obj->getLanguage () . '^^' . $statement->obj->getDatatype () : $statement->obj->getURI ());
		...
		if ($this->use_entities) {
			$this->m_out .= '<!DOCTYPE ' . $this->rdf_qname_prefix . RDF_RDF . ' [' . LINEFEED;
	...
	function writeNamespaceDeclarations() {
		foreach ( $this->m_namespaces as $prefix => $namespace ) {
			...
			if ($prefix == RDF_NAMESPACE_PREFIX && ! $this->rdf_qnames) {
				...
				if ($this->use_entities) {
					$this->m_out .= LINEFEED . INDENTATION . XML_NAMESPACE_DECLARATION_PREFIX . '="&' . $prefix . ';"';
				} else {
					$this->m_out .= LINEFEED . INDENTATION . XML_NAMESPACE_DECLARATION_PREFIX . '="' . $namespace . '"';
				}
			} else {
				if ($prefix == NULL)
					$colon_prefix = $prefix;
				...
				if ($this->use_entities) {
					$this->m_out .= LINEFEED . INDENTATION . XML_NAMESPACE_DECLARATION_PREFIX . $colon_prefix . '="&' . $prefix . ';"';
				} else {
					$this->m_out .= LINEFEED . INDENTATION . XML_NAMESPACE_DECLARATION_PREFIX . $colon_prefix . '="' . $namespace . '"';
				}
			}
		}
	}
	...
	function writeDescriptions() {
		...
		foreach ( $this->m_statements as $key => $statement ) {
			...
			} elseif ($this->canAbbreviateValue ( $object ) && $this->use_attributes && $this->checkForDoubleAttributes ( $predicate )) {
		...
		foreach ( $model->triples as $key => $value ) {
			...
			if ($this->use_entities) {
				...
				if ($value->pred->getURI () == RDF_NAMESPACE_URI . RDF_TYPE)
					$this->collectNamespace ( $value->getObject () );
				elseif (($value->pred->getURI () == RDF_NAMESPACE_URI . RDFS_SUBCLASSOF) || ($value->pred->getURI () == RDF_NAMESPACE_URI . RDFS_SUBPROPERTYOF)) {
					$this->collectNamespace ( $value->getSubject () );
					$this->collectNamespace ( $value->getObject () );
				}
			}
