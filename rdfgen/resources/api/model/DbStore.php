...
class DbStore extends Object {
	 ...
	 * @var array
	 */
	public static $arSupportedDbTypes = array ("MySQL", "MySQLi", "MSSQL", 'MsAccess' );
	...
	function getModel($modelURI) {
		...
		if (! $this->modelExists ( $modelURI ))
			...
			return new DbModel ( $this->dbConn, $modelVars->fields [0], $modelVars->fields [1], $modelVars->fields [2] );
		}
	}
	...
	public static function assertDriverSupported($databaseType) {
		if (! self::isDriverSupported ( $databaseType )) {
			throw new Exception ( 'Unsupported database type, only supported: ' . implode ( ', ', self::$arSupportedDbTypes ) );
		}
		return true;
	} //public static function assertDriverSupported($databaseType)
	...
	function _isSetup_MySQL() {
		...
		if (! $recordSet) {
			...
			if (in_array ( "models", $tables ) && in_array ( "statements", $tables ) && in_array ( "namespaces", $tables )) {
				return true;
			}
		}
		return false;
	...
	function getNamedGraphDb($modelURI, $graphName) {
		...
		if (! $this->modelExists ( $modelURI ))
			...
			return new NamedGraphDb ( $this->dbConn, $modelVars->fields [0], $modelVars->fields [1], $graphName, $modelVars->fields [2] );
		}
	}
	...
	function sparqlQuery($query, $arModelIds = null, $resultform = false) {
		...
		return $engine->queryModel ( null, $this->_parseSparqlQuery ( $query ), $resultform );
