...
class DbModel extends Model {
	...
	function add(&$statement) {
		...
		if (! $this->contains ( $statement )) {
			...
			$sql = "INSERT INTO statements
			        ...
			        VALUES
                    (" . $this->modelID . "," . $this->dbConn->qstr ( $statement->getLabelSubject () ) . "," . $this->dbConn->qstr ( $statement->getLabelPredicate () ) . ",";
			
			if (is_a ( $statement->object (), 'Literal' )) {
				$quotedLiteral = $this->dbConn->qstr ( $statement->obj->getLabel () );
				$sql .= $quotedLiteral . "," . "'" . $statement->obj->getLanguage () . "'," . "'" . $statement->obj->getDatatype () . "'," . "'" . $subject_is . "'," . "'l')";
			} else {
				$object_is = $this->_getNodeFlag ( $statement->object () );
				$sql .= $this->dbConn->qstr ( $statement->obj->getLabel () ) . "," . "''," . "''," . "'" . $subject_is . "'," . "'" . $object_is . "')";
			}
	...
	function find($subject, $predicate, $object) {
		...
		if ((! is_a ( $subject, 'Resource' ) && $subject != NULL) || (! is_a ( $predicate, 'Resource' ) && $predicate != NULL) || (! is_a ( $object, 'Node' ) && $object != NULL)) {
	...
	function replace($subject, $predicate, $object, $replacement) {
		...
		if (((! is_a ( $subject, 'Resource' ) && $subject != NULL) || (! is_a ( $predicate, 'Resource' ) && $predicate != NULL) || (! is_a ( $object, 'Node' ) && $object != NULL)) || (($subject != NULL && is_a ( $replacement, 'Literal' )) || ($predicate != NULL && (is_a ( $replacement, 'Literal' ) || is_a ( $replacement, 'BlankNode' ))))) {
		...
		if (! (! $subject && ! $predicate && ! $object)) {
			...
			$sql = 'UPDATE statements
             SET ';
			if ($subject) {
				$sql .= " subject ='" . $replacement->getLabel () . "', " . " subject_is='" . $this->_getNodeFlag ( $replacement ) . "' ";
				$comma = ',';
			}
			if ($predicate) {
			...
			if ($object) {
				...
				$sql .= $comma . ' object=' . $quotedObject . ", object_is='" . $this->_getNodeFlag ( $replacement ) . "' ";
				if (is_a ( $replacement, 'Literal' )) {
					$sql .= ", l_language='" . $replacement->getLanguage () . "' " . ", l_datatype='" . $replacement->getDataType () . "' ";
				}
			}
		...
		if ($nmsp != '' && $prefix != '') {
			if ($this->_checkNamespace ( $nmsp )) {
				...
				$sql = "INSERT INTO namespaces
                    ...
                    VALUES
                    (" . $this->modelID . ',' . $this->dbConn->qstr ( $nmsp ) . ',' . $this->dbConn->qstr ( $prefix ) . ')';
			}
