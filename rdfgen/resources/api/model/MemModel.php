...
class MemModel extends Model {
	...
	function findRegex($subject_regex, $predicate_regex, $object_regex) {
		...
		foreach ( $this->triples as $value ) {
			if (($subject_regex == NULL || preg_match ( $subject_regex, $value->subj->getLabel () )) && ($predicate_regex == NULL || preg_match ( $predicate_regex, $value->pred->getLabel () )) && ($object_regex == NULL || preg_match ( $object_regex, $value->obj->getLabel () )))
				$res->add ( $value );
		}
	...
	function replace($subject, $predicate, $object, $replacement) {
		...
		if ((! is_a ( $replacement, 'Node' )) || (! is_a ( $subject, 'Resource' ) && $subject != NULL) || (! is_a ( $predicate, 'Resource' ) && $predicate != NULL) || (! is_a ( $object, 'Node' ) && $object != NULL)) {
