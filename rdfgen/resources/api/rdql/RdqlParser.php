...
class RdqlParser extends Object {
	...
	function removeComments($query) {
		...
		for($i = 0; $i <= $last; $i ++) {
			...
			if ($query {$i} == "'" || $query {$i} == '"') {
				$quotMark = $query {$i};
				do
					$clean .= $query {$i ++}; while ( $i < $last && $query {$i} != $quotMark );
				$clean .= $query {$i};
				// language
				if ($query {$i + 1} == '@') {
					do {
				...
				if ($query {$i + 1} == '^' && $query {$i + 2} == '^') {
					do
						$clean .= $query {++ $i}; while ( $i < $last && $query {$i} != ' ' && $query {$i} != "\t" && $query {$i} != "\n" && $query {$i} != "\r" );
				}
	...
	function parseSelect() {
		...
		if (strcasecmp ( 'SELECT', current ( $this->tokens ) ))
			trigger_error ( RDQL_SEL_ERR . "'" . current ( $this->tokens ) . "' - SELECT keyword expected", E_USER_ERROR );
		...
		if (current ( $this->tokens ) == '*') {
			...
			if (strcasecmp ( 'FROM', current ( $this->tokens ) ) && strcasecmp ( 'SOURCE', current ( $this->tokens ) ) && strcasecmp ( 'WHERE', current ( $this->tokens ) ))
				trigger_error ( RDQL_SYN_ERR . ": '" . htmlspecialchars ( current ( $this->tokens ) ) . "' - SOURCE or WHERE clause expected", E_USER_ERROR );
		}
		...
		while ( current ( $this->tokens ) != NULL ) {
			...
			switch ($token) {
				...
				case '(' :
				case ')' :
					trigger_error ( RDQL_SEL_ERR . " '$token' - illegal input", E_USER_ERROR );
					break;
				default :
	...
	function parseWhere() {
		...
		do {
			...
			} elseif (! strcasecmp ( 'USING', current ( $this->tokens ) ) && count ( $this->parsedQuery ['patterns'] ) != 0) {
				if ($comma)
					trigger_error ( RDQL_WHR_ERR . " ',' - unexpected comma", E_USER_ERROR );
				unset ( $this->tokens [key ( $this->tokens )] );
	...
	function replaceNamespacePrefixes() {
		...
		foreach ( $this->parsedQuery ['patterns'] as $n => $pattern ) {
			foreach ( $pattern as $key => $v )
				if ($v ['value'] && $v ['value'] {0} != '?') {
					if (isset ( $v ['is_qname'] )) {
						$this->parsedQuery ['patterns'] [$n] [$key] ['value'] = $this->_replaceNamespacePrefix ( $v ['value'], RDQL_WHR_ERR );
						...
						if (isset ( $this->parsedQuery ['patterns'] [$n] [$key] ['is_literal'] )) {
							if (isset ( $this->parsedQuery ['patterns'] [$n] [$key] ['l_dtype_is_qname'] )) {
								$this->parsedQuery ['patterns'] [$n] [$key] ['l_dtype'] = $this->_replaceNamespacePrefix ( $v ['l_dtype'], RDQL_WHR_ERR );
			...
			foreach ( $this->parsedQuery ['filters'] as $n => $filter )
				foreach ( $filter ['strEqExprs'] as $i => $expr ) {
					if ($expr ['value_type'] == 'QName') {
						$this->parsedQuery ['filters'] [$n] ['strEqExprs'] [$i] ['value'] = $this->_replaceNamespacePrefix ( $expr ['value'], RDQL_AND_ERR );
					...
					elseif ($expr ['value_type'] == 'Literal') {
						if (isset ( $expr ['value_dtype_is_qname'] )) {
							$this->parsedQuery ['filters'] [$n] ['strEqExprs'] [$i] ['value_dtype'] = $this->_replaceNamespacePrefix ( $expr ['value_dtype'], RDQL_AND_ERR );
	...
	function _clearWhiteSpaces() {
		...
		while ( current ( $this->tokens ) == ' ' || current ( $this->tokens ) == "\n" || current ( $this->tokens ) == "\t" || current ( $this->tokens ) == "\r" )
	...
	function _validateVar($token, $clause_error) {
		...
		if (! isset ( $match [0] ) || $match [0] != $token)
			trigger_error ( $clause_error . "'" . htmlspecialchars ( $token ) . "' - variable name contains illegal characters", E_USER_ERROR );
		unset ( $this->tokens [key ( $this->tokens )] );
		return $token;
	}
		...
		if ($token {0} != '<') {
			...
			while ( $token {strlen ( $token ) - 1} != '>' && $token != NULL ) {
				if ($token == '(' || $token == ')' || $token == ',' || $token == ' ' || $token == "\n" || $token == "\r") {
					trigger_error ( $clause_error . '\'' . htmlspecialchars ( $token_res ) . "' - illegal input: '$token' - '>' missing", E_USER_ERROR );
				}
				unset ( $this->tokens [key ( $this->tokens )] );
				$token = current ( $this->tokens );
				$token_res .= $token;
	...
	function _validateLiteral($token) {
		...
		$statement_object = array ('value' => '', 'is_literal' => TRUE, 'l_lang' => '', 'l_dtype' => '' );
		...
		foreach ( $this->tokens as $k => $token ) {
			...
			// parse @language(^^datatype)?
				...
				if (strpos ( $lang, '^^' ) || substr ( $lang, 0, 2 ) == '^^') {
					...
					if (! $dtype)
						trigger_error ( RDQL_WHR_ERR . $quotation_mark . $statement_object ['value'] . $token . " - datatype expected", E_USER_ERROR );
				...
				if (! $lang)
					trigger_error ( RDQL_WHR_ERR . $quotation_mark . $statement_object ['value'] . $token . " - language expected", E_USER_ERROR );
	...
	function _validatePrefix($token) {
		...
		if (! $this->_validateNCName ( $token ))
			trigger_error ( RDQL_USG_ERR . "'" . htmlspecialchars ( $token ) . "' - illegal input, namespace prefix expected", E_USER_ERROR );
		unset ( $this->tokens [key ( $this->tokens )] );
		return $token;
	}
