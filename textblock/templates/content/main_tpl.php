...
$txtShowBlocks = $this->objLanguage->languageText('mod_textblock_showblocks', 'textblock');
$txtNext = $this->objLanguage->languageText('word_next');
$txtPrev = $this->objLanguage->languageText('word_prev');


$pageNav = " <span class='page' style='float:right'>$txtShowBlocks <b>$start - $end of $total</b><br/>";
if ($page > 1) {
	$prevPage = $page - 1;
	$pageNav .= " <a href='?module=textblock&pagenum=$prevPage'>&lt;&lt; $txtPrev</a>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
}
if ($end < $total) {
	$nextPage = $page + 1;
	$pageNav .= "<a href='?module=textblock&pagenum=$nextPage'>$txtNext &gt;&gt;</a>";
}
$pageNav .= "</span>";

$leftHelp = "<span style='float:left'>".$objLanguage->languageText("mod_textblock_leftside",'textblock')."</span>";
$rightSideColumn = $this->objH->show().$leftHelp.$pageNav.'<hr/>';
...
$tableHd = array();
$tableHd[] = $objLanguage->languageText('mod_textblock_blockname','textblock');
...
$this->Table->addHeader($tableHd, "heading", "align='left'");

//Loop through and display the records
$rowcount = 0;
foreach ($ar as $line) {
	$oddOrEven = ($rowcount % 2 == 0) ? "odd" : "even";
	$rowcount++;
    ...
    $editLink=$this->uri(array('action' => 'edit', 'id' =>$line['id']));
    ...
    $delLink = $this->uri(array('action' => 'delete', 'confirm' => 'yes', 'id' => $line['id']));
    $objConfirm = $this->newObject('confirm','utilities');
    
	$rep = array('ITEM', $line['id']);
    $objConfirm->setConfirm($objDelIcon->show(), $delLink, $this->objLanguage->code2Txt("mod_textblock_conf",'textblock', $rep));
