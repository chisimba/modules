<?php
/**
 * asciiart controller class
 *
 * Class to control the asciiart module
 *
 * PHP version 5
 *
...
class aart extends controller
{
    ...
    public function init()
    {
        try {
            ...
            $this->objOps        = $this->getObject('asciiart', 'utilities');
    ...
    public function dispatch() {
        ...
        switch ($action) {
            case NULL:
                return 'enterurl_tpl.php';
                break;
            
            case 'convert' :
                $url = $this->getParam('url');
                $this->objOps->color = TRUE;
                $this->objOps->getUrl($url);
                $img = $this->objOps->draw();
                $this->objOps->destroy();
                $this->setVarByRef('img', $img);
    ...
    function requiresLogin($action='') {
        $allowedActions = array('','convert', NULL);
