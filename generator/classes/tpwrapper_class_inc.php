...
*
*/
class tpwrapper extends object
{
    /**
    * 
    ...
    */
    function show()
    {
        ...
        $myTable->startRow();
        $myTable->addCell($this->objLanguage->languageText("mod_generator_controller_module", "generator"));
        $myTable->addCell($this->__getModuleElement());
        $myTable->endRow();
        
        //Create an element for the input of table name and add it to the table
        $myTable->startRow();
        $myTable->addCell($this->objLanguage->languageText("mod_generator_controller_classfilename", "generator"));
        $myTable->addCell($this->__getFileNameElement());
        $myTable->endRow();
        
        //Create an element for the input of modulename and add it to the table
        $myTable->startRow();
        $myTable->addCell($this->objLanguage->languageText("mod_generator_controller_genclassname", "generator"));
        $myTable->addCell($this->__getGenClassElement());
        $myTable->endRow();
        
        //Create an element for the input of modulename and add it to the table
        $myTable->startRow();
        $myTable->addCell($this->objLanguage->languageText("mod_generator_controller_params", "generator"));
        $myTable->addCell($this->__getParamsElement());
        $myTable->endRow();
    ...
    * 
    */ 
    private function __getFileNameElement()
    {
        //Create an element for the input of module code
        $objElement = new textinput ("filename");
        //Set the field type to text
        $objElement->fldType="text";
        $objElement->size=40;
        if (isset($this->filename)) {
            $objElement->value=$this->filename;
        }
        //Add the $title element to the form
        return $objElement->show();
    }
    ...
    * 
    */ 
    private function __getGenClassElement()
    {
        //Create an element for the input of module code
    ...
    * 
    */ 
    private function __getParamsElement()
    {
        //Create an element for the input of module code
        $objElement = new textinput ("params");
        //Set the field type to text
        $objElement->fldType="text";
        $objElement->size=40;
        if (isset($this->params)) {
            $objElement->value=$this->params;
        }
        //Add the copyright element to the form
        return $objElement->show();
    }
