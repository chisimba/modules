...
//-------------------------THE END---------------------//
	...
	*/
	...
	case 'search':
	$this->objSearch =& $this->getObject('coursesearch','klorclient');
	$coursenameObjAlphabet=& $this->getObject('alphabet','navigation');
	$coursenameListFieldset =& $this->newObject('fieldset', 'htmlelements');
	$coursenameListFieldset->setLegend('legend');
	//$coursenameListFieldset->width="80%";
	//add the link to return to Admin Page
	$returnLink = new link($this->uri(array('action'=>'fileList')));
	$returnLink->link = $this->objLanguage->languageText('word_back');
	//$coursenameListFieldset->addContent('<p>'.$surnameObjAlphabet->putAlpha('fuck').'</p>');
	echo $this->objSearch->show();
	/**
	* browse by coursename
	* alphabet object
	*/
		
	//prepare items to be used with the alphabet object
	//$surnameLinkArray=array('action'=>'listpatients','searchHow'=>'surname','searchGender'=>'both','searchLetter'=>'LETTER');
	//$surnameUrl=$this->uri($surnameLinkArray,'joostedispensing');
	//list fieldset
	//add alphabet to fieldset
	//echo $this->objSearch->show().'<br />';
	//echo $coursenameListFieldset->show().'<br />&nbsp;&nbsp;'; 
	...
	case 'fileList':
		...
		$tblCourseWare->addHeaderCell($lblpreview);
		$tblCourseWare->addHeaderCell($courseDownload);
		$tblCourseWare->addHeaderCell($lblmeta);
		$tblCourseWare->addHeaderCell($lblrating);
		...
		{
			//new link $this->objConfig->siteRoot().
			...
			$name = $item['name'];
			//print $path;
			$clean_path = $this->objConfig->siteRoot().$this->objConfig->contentPath().'klor/course-'.$name;
			...
			//add icon for metadata
			//download icon	
			$icon = $this->getObject('geticon','htmlelements');
			$icon->setIcon('add');
			$lblmeta = "Insert Metadata";	
			$icon->alt = $lblmeta;
			$icon->align=false;
		...
		$uriRating  = "<a href=\"{$uriRating}\">".$rating_icon->show()."</a>";
		...
		$tblCourseWare->startRow($rowClass);
			...
			//$tblCourseWare->addCell("<a href=\"{$preview}\">".$viewicon->show()."</a>");
