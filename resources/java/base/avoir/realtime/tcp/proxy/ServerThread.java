    ...
    private void removeLock(String userId) {
        for (int i = 0; i < presentationLocks.size(); i++) {
            if (userId.equals(presentationLocks.elementAt(i).getUserId())) {
                System.out.println("Lock removed too");
                presentationLocks.remove(i);
            }
        }
...
//                    ex.printStackTrace();
                ...
                if (obj instanceof LauncherAckPacket) {
                    ...
                    System.out.println("Launcher " + lack.getUser() + " signing in");
                ...
                if (obj instanceof SessionListingRequestPacket) {
                    System.out.println("Requesting session listing ...");
                ...
                if (packet != null) {
                    ...
                    if (packet instanceof AckPacket) {
                        ...
                        System.out.println("Client " + thisUser + " signing in");
    ...
    private void processSessionListingRequest() {
        System.out.println("Sedning it back to the newly logged in ...");
        try {
    ...
    /**
     * Monitor users every minute and remove dull ones
     * including those who have locked presentations
     */
    private void monitorUsers() {
        while (true) {
            delay(1000);
            testAndRemoveDisconnectedUsers();
            //broadcast

            try {
                ClientPacket clientPacket = new ClientPacket(getUsersBySessionId(thisUser.getSessionId()));
                broadcastPacket(clientPacket);
            } catch (Exception ex) {
                // ex.printStackTrace();
            }
        }
    ...
    private void testAndRemoveDisconnectedUsers() {
        for (int i = 0; i < clients.size(); i++) {
            try {
                ...
                ex.printStackTrace();
                System.out.println("User " + clients.nameAt(i) + " is dead..removing");
                // user is dead....remove and update eveyone else
                clients.removeUser(clients.nameAt(i));
                updateMonitors();
                //remove lock too
                removeLock(clients.nameAt(i).getUserName());
    ...
    private void processModuleFilePacketRequest(ModuleFileRequestPacket packet) {
        ...
        if (!slideServerFound) {
            System.out.println("Slide server not found!!");
        }
    }
