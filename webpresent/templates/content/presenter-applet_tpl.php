...
var Java  = PluginDetect.isMinVersion('Java', '1.5');
...
// -----------------------------------------------------------------------------------
// Given one or more installed JRE versions, display the highest one.


document.write(\"Highest Installed Version: \" + PluginDetect.getVersion('Java', 'getJavaInfo.jar') + '<br>')
...
if (Java == 1){
  ...
  document.write('Java version is < ' + minVersion + ' or not installed/enabled<br>')
...
var all = PluginDetect.java.allVersions;  // an array

if (all.length>1){

  document.write('Deployment Toolkit has detected multiple Java versions: ' + all.join(" & ") + '<br>');


};

document.write('this is a ts');
...
if(minVer == 1){
window.open("';
$jsContent.=$this->objConfig->getSiteRoot().'/index.php?module=webpresent&action=showapplet","_self","fullscreen"); 
//alert(<?php echo("Location: ".$this->objConfig->getSiteRoot()."index.php?module=webpresent&action=showapplet");?>);
}else{
document.write("Minimum java 1.5 required");
}
//--></script>';
