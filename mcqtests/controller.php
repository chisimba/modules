...
class mcqtests extends controller
{
    ...
    */
    public function init()
    {
        ...
        $this->objAttachments=&$this->newObject('dbattachments','email');
           ...
           case 'delete':
            ...
            case 'applyaddquestion':
                ...
                if($postSave == $this->objLanguage->languageText('mod_mcqtests_saveaddanotherquestion')){
                    return $this->nextAction('addquestion',array('id'=>$id, 'count'=>$this->getParam('qOrder')));
                }
            ...
            case 'addanswer':
                ...
                return $this->nextAction('addanswers', array('questionId'=>$questionId,
                    'count'=>$this->getParam('count')));
            ...
            case 'applyaddanswer':
                ...
                if($postSave == $this->objLanguage->languageText('word_cancel')){
                    return $this->nextAction('editquestion',array('questionId'=>$postQuestionId));
                }
                $this->addAnswers($postTestId, $postQuestionId, $_POST);
                $msg = $this->objLanguage->languageText('mod_mcqtests_confirmaddanswer', 'mcqtests');
                $this->setSession('confirm', $msg);
                return $this->nextAction('editquestion',array('questionId'=>$postQuestionId, 'confirm'=>'yes'));
            ...
            case 'deleteanswer':
                ...
                return $this->nextAction('editquestion',array('questionId'=>$this->getParam('questionId')));
            ...
            case 'doexport':
                ...
                if($exportType=='answers'){
                    ...
                    if(isset($usersResultList) && !empty($usersResultList)){
                        ...
                        fclose($outputFile);
                        return $this->nextAction('emailresults',array('file'=>$file,'testId'=>$testId));
                    }else{
                        return $this->nextAction('');
                    }
                }else{
            ...
            case 'emailresults':
                ...
                $array=array('filename'=>basename($file),'item'=>$testData[0]['name']);
            ...
            case 'applyaddlab':
                ...
                if($file['comLab']['type']!='text/x-comma-separated-values'){
                    return $this->nextAction('addlab',array('id'=>$id,'mode'=>$mode,'error'=>TRUE));
                }else{
                    $contentRoot=$this->objConfig->getcontentBasePath();
                    $fileLocation=$contentRoot.'/modules/mcqtests/';
                    if(!is_dir($fileLocation)){
                    ...
                    if(file_exists($labFileLocation)){
                    }
                    move_uploaded_file($file['comLab']['tmp_name'],$labFileLocation);
                    if($mode=='add'){
                        return $this->nextAction('addtest',array('mode'=>$mode),'mcqtests');
                    }else{
                        return $this->nextAction('edit',array('id'=>$id,'mode'=>$mode),'mcqtests');
                    }
                }
                break;
    ...
    */
    private function applyAddTest()
    {
        ...
        if(isset($postEqual) && !empty($postEqual)){
            ...
            $arrField = array('percentage' => $percent);
    ...
    * @return The links.
    */
    private function generateLinks($current, $total, $num = 4)
    {
