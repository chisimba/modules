...
if(!empty($data)){
    ...
    foreach($data as $line){
        ...
        $objLayer1->str = '<b>'.$questionLabel.' '.$line['questionorder'].':</b>&nbsp;&nbsp;&nbsp;'.$line['question'];
...
if($qNum < $data[0]['count']){
    $objLink->link($this->uri(array('action'=>'showtest', 'id'=>$result['testid'], 'qnum'=>$qNum, 'studentId'=>$result['studentid'])));
    $objLink->link = $nextLabel;
    $links = $objLink->show().'&nbsp;&nbsp;|&nbsp;&nbsp;';
}
$objLink->link($this->uri(array('action'=>'liststudents', 'id'=>$result['testid'])));
