...
//=========Decorating the date============
        ...
        if ($this->showFullName) {
            ...
            $return .= '<div>' . $dateSpan . '<br/> <strong>' . $post['firstname'] . ' ' . $post['surname'] . '</strong>' . stripslashes($post['post_title']) . ' <img src="skins/tu/icons/date-time.png" > <strong>' . $this->objDateTime->formatDateOnly($post['datelastupdated']) . '</strong> ' . strtolower($this->objLanguage->languageText('word_at', 'system')) . ' ' . $this->objDateTime->formatTime($post['datelastupdated']) . ' (' . $this->objTranslatedDate->getDifference($post['datelastupdated']) . ')  <br/><br/> </div>';
        } else {
            // Start of the Title Area
