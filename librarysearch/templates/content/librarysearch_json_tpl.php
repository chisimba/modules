...
if (!empty($arrMaps)){
    ...
    foreach ($arrMaps as $map){
        ...
        */
        ...
        if ($map['is_dynamic'] == '1') {
            $objIcon->setIcon('checked', 'gif', 'icons/lib/');
        } else {
            $objIcon->setIcon('unchecked', 'gif', 'icons/lib/');
        }
        ...
        $editIcon = "<a id='edit_$map[id]' title='Edit URL Mapping' href='http://localhost/fresh/?module=lib&action=getform&type=addeditform&id=$map[id]'>".$objIcon->show()."</a>";
        //} else {
        //    $editIcon = '';
        //}
