...
class groups extends dbTable
{
    ...
    */
    public function addGroups($surveyId)
    {
        ...
        $observerId=$this->objGroupAdmin->addGroup('Observers','Observers of the survey',$groupId);
        if($observerId===FALSE){
            $isSuccessful=FALSE;
            return $isSuccessful;
        }
        $collaboratorId=$this->objGroupAdmin->addGroup('Collaborators','Collaborators on the survey',$groupId);
        if($observerId===FALSE){
            $isSuccessful=FALSE;
            return $isSuccessful;
        }
        $respondentId=$this->objGroupAdmin->addGroup('Respondents','Respondents of the survey',$groupId);
        if($respondentId===FALSE){
            $isSuccessful=FALSE;
            return $isSuccessful;
        }
        ...
        foreach($arrAssignList as $userId=>$assign){
            ...
            if($assign=='Observers'){
                if($inGroup!='Observers'){
                    $this->objGroupAdmin->addGroupUser($arrGroupIdList['Observers'],$pkId);
                    $this->objGroupAdmin->deleteGroupUser($arrGroupIdList['Collaborators'],$pkId);
                    $this->objGroupAdmin->deleteGroupUser($arrGroupIdList['Respondents'],$pkId);
                }
            }elseif($assign=='Collaborators'){
                if($inGroup!='Collaborators'){
                    $this->objGroupAdmin->addGroupUser($arrGroupIdList['Collaborators'],$pkId);
                    $this->objGroupAdmin->deleteGroupUser($arrGroupIdList['Observers'],$pkId);
                    $this->objGroupAdmin->deleteGroupUser($arrGroupIdList['Respondents'],$pkId);
                }
            }elseif($assign=='Respondents'){
                if($inGroup!='Respondents'){
                    $this->objGroupAdmin->addGroupUser($arrGroupIdList['Respondents'],$pkId);
                    $this->objGroupAdmin->deleteGroupUser($arrGroupIdList['Observers'],$pkId);
                    $this->objGroupAdmin->deleteGroupUser($arrGroupIdList['Collaborators'],$pkId);
                }
            }else{
                foreach($arrGroupIdList as $group=>$groupId){
                    $this->objGroupAdmin->deleteGroupUser($groupId,$pkId);
                }
            }
        }
    }
    ...
    */
    ...
    {
        $groupId=$this->objGroupAdmin->getLeafId(array('Surveys',$surveyId,$group));
    ...
    */
    public function assignGroups($surveyId,$arrGroupAssignList)
    {
        foreach($arrGroupAssignList as $groupId=>$group){
            $arrAssignList=array();
            $arrGroupMembers=$this->objGroupAdmin->getSubGroupUsers($groupId,array('userId','username'));
