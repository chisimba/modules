...
*/
class dbstudentprogramme extends dbTable
{
    ...
    */
	...
	function connect(){
		//$this->remotedb =& $this->getObject('remotedb','remotedatasource');
		//return $this->remotedb->connectRemotely('tbl_studentprogramme');
        //Uses NUSOAP
        require_once("lib/nusoap/nusoap.php");
        $this->NAMESPACE="http://172.16.65.134/webserviceDEV/studentinfo2.php?wsdl";
	...
	function getStudentProgramme($studentId = null){
	/*	$connection = $this->connect();
		
		$sql = "select sp.programmeid,p.fucultyID, p.programme from tbl_studentprogramme sp inner join tbl_programme p on sp.programmeid = p.programmeID";
		
		if(is_null($studentId)){
			$filter = null;
		}
		else{
			$filter = " where sp.studentid = '$studentId'";
		}
