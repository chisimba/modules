...
class modulesAdmin extends modules
{
    ...
    var $objTableInfo; //handle for tableinfo class object
    var $objKeyMaker; //handle for primary key generation
    var $MODULE_ID; // replaces use of defined constant.
    var $MODULE_NAME;
    var $MODULE_DESCRIPTION;
    var $update=FALSE;

    var $output=''; // for any feedback messages from internal functions.
    var $errorText=''; // for any feedback messages from internal functions.
    ...
    */
    ...
    /**
    * This is a method to check if a dependent module is installed. If it not, it ends
    * execution.
    * @param string $moduleId The moduleId of the module on which the
    * current module depends
    * @returns boolean TRUE|FALSE
    */
    function checkDependency($moduleId) {
        if ($this->checkIfRegistered(NULL,$moduleId)) {
            return TRUE;
        } else {
            //$objkngError->endon_error("mod_dependencyviolation", $module, FALSE);
            return FALSE;
        }
    ...
    */
    function localinsert($data,$table='tbl_modules')
    {
        $sql='INSERT INTO '.$table.' SET ';
        $sql.=$this->makeSQLParams($data);
        $this->executeModSQL($sql);
    }


    /**
    * This is a method to edit without use of replication
    * @author James Scoble
    * @param $table string name of table
    * @param $data  - assoc array
    */
    function localEdit($data,$where,$table='tbl_modules')
    {
