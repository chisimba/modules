...
public class RealtimeOptionsFrame extends javax.swing.JFrame {
    ...
    private AudioOptionsPanel audioOptionsPanel;

    /** Creates new form RealtimeOptionsFrame */
    public RealtimeOptionsFrame(ClassroomMainFrame mf, int selectedTabIndex) {
        this.mf = mf;
        initComponents();
        audioOptionsPanel = new AudioOptionsPanel(mf);
        tabbedPane.setSelectedIndex(selectedTabIndex);
        directOpt.setSelected(mf.getRealtimeOptions().useDirectConnection());
        manualOpt.setSelected(mf.getRealtimeOptions().useManualProxy());
        systemOpt.setSelected(mf.getRealtimeOptions().useSystemProxy());
        cachingOpt.setSelected(mf.getRealtimeOptions().useCache());
        onlineSNOpt.setSelected(true);
        otherSNHostField.setText("127.0.0.1");
        otherSNPortField.setText("22225");
        proxyHostField.setText(mf.getRealtimeOptions().getProxyHost());
        buttonGroup1.add(directOpt);
        buttonGroup1.add(manualOpt);
        buttonGroup1.add(systemOpt);
        proxyPortField.setText(mf.getRealtimeOptions().getProxyPort());
        boolean browserUsingProxy = mf.getRealtimeOptions().isBrowserUsingProxy();
//       browserDirectOpt.setSelected(browserUsingProxy);
        updateVoiceServiceTextStatus();
        tabbedPane.addTab("Audio Settings", audioOptionsPanel);
    }
    ...
    private void okButtonActionPerformed(java.awt.event.ActionEvent evt) {
        mf.getRealtimeOptions().setProxyHost(proxyHostField.getText());
        mf.getRealtimeOptions().setProxyPort(proxyPortField.getText());
        mf.getRealtimeOptions().setUseDirectConnection(directOpt.isSelected());
        mf.getRealtimeOptions().setUseManualProxy(manualOpt.isSelected());
        mf.getRealtimeOptions().setUseSystemProxy(systemOpt.isSelected());
        if (localSNOpt.isSelected()) {
//            mf.getTCPClient().setSuperNodeHost(otherSNHostField.getText());
            //          mf.getTCPClient().setSuperNodePort(Integer.parseInt(otherSNPortField.getText().trim()));
        }
        //    client.refresh();
        dispose();
    ...
    private void saveChanges() {
        ...
        mf.getRealtimeOptions().setUseManualProxy(manualOpt.isSelected());
        mf.getRealtimeOptions().setUseSystemProxy(systemOpt.isSelected());
        mf.getRealtimeOptions().setUseCache(cachingOpt.isSelected());
        mf.getRealtimeOptions().setAudioMicPort(audioOptionsPanel.getAudioMicPort());
        mf.getRealtimeOptions().setAudioSpeakerPort(audioOptionsPanel.getAudioSpeakerPort());
        mf.getRealtimeOptions().setAudioServerHost(audioOptionsPanel.getAudioServerHost());
    }
    ...
    private void clearCache() {
        String cache = avoir.realtime.common.Constants.getRealtimeHome() + "/presentations/";
    ...
    private void onlineSNOptActionPerformed(java.awt.event.ActionEvent evt) {
        mf.getRealtimeOptions().setUseOnlineSuperNodes(true);
    }

    private void localSNOptActionPerformed(java.awt.event.ActionEvent evt) {
        mf.getRealtimeOptions().setUseOnlineSuperNodes(false);
    }
