...
public class SlideProcessor {
    ...
    public SlideProcessor(ServerThread serverThread, Vector<SlideServer> slideServers) {
        ...
        synchronized (serverThread.getClients()) {
            ...
            for (int i = 0; i < serverThread.getClients().size(); i++) {
                if (serverThread.getClients().nameAt(i).getSessionId().equals(p.getSessionId())) {
                    try {
                        serverThread.getClients().elementAt(i).writeObject(p);
                        serverThread.getClients().elementAt(i).flush();
