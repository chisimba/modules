...
public class RPacketListener implements PacketListener {
    ...
    private static void processRealtimePacket(RealtimePacket packet) {
        ...
        try {
            ...
            if (mode.equals(Mode.USERLIST_ACCESSLEVEL_REPLY)) {
                ...
                Startup.realtimeFxInt.getRoomMemberListFrame().setMembers(memberList);
                ...
                Startup.realtimeFxInt.getRoomListFrame().populateRooms(roomList);
                ...
                Startup.realtimeFxInt.getWhiteboard().paintPointer(pointer.getPointer(),
            ...
            // JOptionPane.showMessageDialog(null, success ? successMessage : errorMessage);
                ...
                for (int i = 0; i < fileView.size(); i++) {
                    ...
                    if (!new File(local).exists() || needsUpdate) {
                        ...
                        Startup.realtimeFxInt.getChatRoomManager().requestNewSlides(GeneralUtil.getExt(rFile.getFilePath()));
                    } else {
                ...
                try {
                    ...
                    if (item.isNewItem()) {
                        Startup.realtimeFxInt.getWhiteboard().addItem(item);
                    } else {
                        Startup.realtimeFxInt.getWhiteboard().updateItem(item);
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
               ...
               Startup.realtimeFxInt.getWhiteboard().updateText(content, id);
                ...
                if (item.getType().equals("line")) {
                    Startup.realtimeFxInt.getWhiteboard().updateItemPostion(item.getX(), item.getY(), item.getW(), item.getH(), item.getId());
                } else {
                    Startup.realtimeFxInt.getWhiteboard().updateItemPostion(item.getX(), item.getY(), item.getId());
                }
    ...
    private static void processMessage(Message message) {
        ...
        if (type == null) {
            ...
            Startup.realtimeFxInt.getChatRoomManager().getChatRoom().insertParticipantName("(" + from + ") ");
            Startup.realtimeFxInt.getChatRoomManager().getChatRoom().insertParticipantMessage(from, message.getBody(), size, color);
            Startup.realtimeFxInt.getChatRoomManager().getChatRoom().insertSystemMessage(sentDate + "\n");

        } else if (type.equals("sys-text")) {
            Startup.realtimeFxInt.getChatRoomManager().getChatRoom().insertSystemMessage(message.getBody());

        } else if (type.equals("istyping")) {
            // Startup.realtimeFxInt.setTyping(message.getBody().trim().equals(""));
        } else {
