...
class dbrealtimepresentationslides extends dbtable
{
    ...
    */
    ...
    {
        parent::init('tbl_realtimepresentation_slides');
    ...
    {
        // Setup Directory
        $dir = $this->objConfig->getcontentBasePath().'realtimepresentation/'.$id;
    ...
    {
        $full = $this->objConfig->getcontentBasePath().'realtimepresentation_slide_thumbnails/'.$slideId.'.jpg';
        $rel = $this->objConfig->getsiteRoot().$this->objConfig->getcontentPath().'realtimepresentation_slide_thumbnails/'.$slideId.'.jpg';
    ...
    private function generateSlideThumbnail($fileId, $slideId, $order)
    {
        ...
        $destination = $this->objConfig->getcontentBasePath().'realtimepresentation_slide_thumbnails/'.$slideId.'.jpg';
        ...
        if (file_exists($destination)) {
            ...
            // Get Source Image
            $source = $this->objConfig->getcontentBasePath().'realtimepresentation/'.$fileId.'/img'.($order-1).'.jpg';
            ...
            if (file_exists($source)) {
                ...
                $destinationDir = $this->objConfig->getcontentBasePath().'/realtimepresentation_slide_thumbnails';
    ...
    public function getPresentationSlidesContent($id, $withSlideShow = FALSE)
    {
        ...
        foreach ($slides as $slide)
        {
            if ($withSlideShow)
            {
                ...
                $slideShow[] = "viewer.add('usrfiles/realtimepresentation/".$id."/img".($counter-1).".jpg', '".(addslashes($slide['slidetitle']))."');";
            } else {
    ...
    public function deleteSlides($fileId)
    {
        ...
        if (count($slides) > 0)
        {
            foreach ($slides as $slide)
            {
                ...
                $firstPage =  $this->objConfig->getcontentBasePath().'realtimepresentation/'.$fileId.'/'.$fileId.'.html';
                if (file_exists($firstPage))
                {
                    unlink($firstPage);
                }

                $img = $this->objConfig->getcontentBasePath().'realtimepresentation/'.$fileId.'/img'.($slide['slideorder']-1).'.jpg';
                if (file_exists($img))
                {
                    unlink($img);
                }

                $html = $this->objConfig->getcontentBasePath().'realtimepresentation/'.$fileId.'/img'.($slide['slideorder']-1).'.html';
                if (file_exists($html))
                {
                    unlink($html);
                }

                $text = $this->objConfig->getcontentBasePath().'realtimepresentation/'.$fileId.'/text'.($slide['slideorder']-1).'.html';
                if (file_exists($text))
                {
                    unlink($text);
                }
    ...
    {
        $fullPath = $this->objConfig->getcontentBasePath().'realtimepresentation_slide_thumbnails/'.$slideId.'.jpg';
