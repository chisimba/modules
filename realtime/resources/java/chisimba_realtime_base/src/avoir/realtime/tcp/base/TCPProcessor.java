...
public class TCPProcessor {
    ...
    public TCPProcessor(TCPClient tcpClient, RealtimeBase base) {
        this.tcpClient = tcpClient;
        this.base = base;
    }

    public TCPProcessor(TCPClient tcpClient) {
        this.tcpClient = tcpClient;

    }

    public TCPProcessor(TCPClient tcpClient, ClientAdmin clientAdmin) {
        this.tcpClient = tcpClient;
    ...
    public void processClassromFilesLog(ClassroomFileLog packet) {
        ...
        for (int i = 0; i < log.size(); i++) {
            ...
            if (p.getType() == Constants.FLASH) {
                filePath = Constants.getRealtimeHome() + "/classroom/flash/" + base.getSessionId() + "/" + p.getPath();
