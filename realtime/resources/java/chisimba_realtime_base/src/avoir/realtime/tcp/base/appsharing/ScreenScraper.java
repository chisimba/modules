...
public abstract class ScreenScraper implements Runnable {
    ...
    public ScreenScraper(RealtimeBase base) {
        appView = new AppView(PixelUtil.DEFAULT_WIDTH, PixelUtil.DEFAULT_HEIGHT,base);
        this.base = base;
...
//    	System.out.println("saying to wait " + this.iNextWait + " millis");
    ...
    public void run() {
        try {
            ...
            bScrapeRequested = true;
            int iMode = 0;
            base.getBaseManager().setScreenCapture(true);
            while (bContinue) {
                if (bScrapeRequested) {
    ...
    private void doScrape()
            throws Exception {
        Rectangle fullScrapeRect = new Rectangle(ss);
...
//            this.updateBandwidthMetric(data.getByteCount());
           // appView.pixelUpdate(data);
            ...
            base.getTcpClient().sendPacket(new DesktopPacket(data, base.getSessionId()));
