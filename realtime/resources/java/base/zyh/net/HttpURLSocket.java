...
/**
 * HttpURLSocket class implements client sockets through http url proxy server
 */
public class HttpURLSocket extends Socket
{
	/**
	...
	/*
	 * The factory for all client HttpURLSockets.
	 */
	private static HttpURLSocketImplFactory factory=null;
	/*
	 * The implementation of this Socket.
	 */
	private HttpURLSocketImpl impl;
	 ...
	 *			   socket factory.
	 */
	public static synchronized void setSocketImplFactory(HttpURLSocketImplFactory fac)throws IOException
	{
		factory = fac;
	}
	 ...
	 */
	protected HttpURLSocket()
	{
		impl = (factory != null) ? (HttpURLSocketImpl)factory.createSocketImpl() : new HttpURLSocketImpl();
	}
	 ...
	 */
	protected HttpURLSocket(HttpURLSocketImpl impl)
	{
		this.impl = impl;
	}
	 ...
	 */
	public HttpURLSocket(String host, int port) throws IOException
	{
	 ...
	 * @exception  IOException  if an I/O error occurs when creating the socket.
	 */
	public HttpURLSocket(InetAddress address, int port) throws IOException
	{
	 ...
	 * @exception  IOException  if an I/O error occurs when creating the socket.
	 */
	public HttpURLSocket(String host, int port, InetAddress localAddr, int localPort) throws IOException
	{
		this();
		try
		{
	 ...
	 * @exception  IOException  if an I/O error occurs when creating the socket.
	 */
	public HttpURLSocket(InetAddress address, int port, InetAddress localAddr,int localPort) throws IOException
	{
		this();
		try
		{
...
/*		try
	...
	{
		return "HttpURLSocket[addr=" + impl.getInetAddress() +
			",port=" + impl.getPort() +
			",localaddr=" 
			+(impl.getLocalAddress()!=null?impl.getLocalAddress().toString():"127.0.0.1")
			+",localport=" + impl.getLocalPort()+
			(impl.getHttpURLProxyURL()!=null?
			",httpURLProxyURL=" + impl.getHttpURLProxyURL():"")
			+"]";
	}

	/**
	 * Some importmant optional parameters for HTTP url proxy server
	 * ***Note:***
	 * HTTP url proxy server is not http proxy.
	 * HTTP url proxy is a socket relay server which runs on the server.
	 */
	/**
	 * Use USER or USERNAME to set username for the specific HTTP url proxy serer.
	 */
	public static final String USER ="USER";
	public static final String USERNAME =USER;

	/**
	 * Use USERID,PASSWD or PASSWORD to set password for the specific HTTP url proxy serer.
	 */
