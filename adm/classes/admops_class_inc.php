...
class admops extends object
{
	...
	public function parsemail()
	{
		...
		try {
			...
			if(!$this->conn)
			{
				log_debug("No route to mail host");
				exit;
			}
				...
				if(!empty($datum['attachments']))
				{
					if(is_array($datum['attachments']))
					{
						foreach($datum['attachments'] as $files)
						{
							...
							$tparts = explode("/", $type);
							//print_r($tparts); die();
							//log_debug($filename." is ".$type);

							if($tparts[0] == "text" && $tparts[1] == "plain")
							{
								log_debug("found $filename for ADM!!!");

								$filesarray[] = $fullpath.$filename;
	...
	public function sendLog()
	{
		...
		if(file_exists($path) && filesize($path) > 0)
		{
			...
			if ($objMailer->send()) {
				...
				return $this->objLanguage->languageText("mod_adm_sqldatanotsent", "adm");
			}
		}
		else {
