...
class dbreports extends dbTable {
  ...
  function get_students_information($regno,$year_id){
    ...
    function get_student_marks($regno,$exam,$term,$year_id) {
       $sql="SELECT `subject_name`,exam_type,`score`  FROM `tbl_result`,tbl_subjects,tbl_exam WHERE tbl_subjects.`puid`=tbl_result.`tbl_subjects_id`
       ...
       $subject_marks=$this->queryRow($sql);
        if($subject_marks){
