...
$grpMembers = $this->_objGroupAdmin->getGroupUsers($group, array('userid', 'firstName', 'surname', 'title', 'emailAddress', 'country', 'sex', 'staffnumber'), $filter);
    ...
    foreach ($grpMembers as $grpMember)
    {
        ...
        if ($this->isValid('removeuser')) {
            $string = str_replace(
                ...
                $this->objLanguage->languageText('phrase_eportfolio_confirmdeleteuser', 'eportfolio')
            );
            //    public function getDeleteIconWithConfirm($id, $deleteArray=NULL, $callingModule=NULL, $deletephrase='phrase_confirmdelete')
            $deleteicon = $objIcon->getDeleteIconWithConfirm(NULL, array('action'=>'removeuser', 'userid'=>$grpMember['userid'], 'group'=>$group), 'eportfolio', $string);
