...
$info= '<div style="padding:10px;">'.$header->show();

$required = '<span class="warning"> * '.$this->objLanguage->languageText('word_required', 'system', 'Required').'</span>';

$str ='Register for '.$sessionTitle;
$str = str_replace('[[SITENAME]]', $this->objConfig->getSitename(), $str);

$info.= '<p>'.$str.'<br />';
$info.= $this->objLanguage->languageText('mod_userregistration_pleaseenterdetails', 'userregistration', 'Please enter your details, email address and desired user name in the form below.').'</p>';

$form = new form ('register', $this->uri(array('action'=>'register','sessionid'=>$sessionid)));
...
if ($mode == 'addfixup' && count($messages) > 0) {
    $errors.= '<ul><li><span class="error">'.$this->objLanguage->languageText('mod_userdetails_infonotsavedduetoerrors', 'userdetails').'</span>';

   $errors.= '<ul>';
        foreach ($messages as $message)
        {
            if ($message != '') {
                $errors.= '<li class="error">'.$message.'</li>';
            }
        }
