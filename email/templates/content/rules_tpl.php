...
//    $objTable->cellspacing='2';
    ...
    $objForm=new form('rulesform',$this->uri(array('action'=>'saverule','mode'=>$mode)));
    $objForm->addToForm($ruleFieldset);
    $objForm->addToForm($buttons);
    $objForm->addToForm($hiidenInput);
    $forms=$objForm->show();

    $objForm=new form('hiddenform',$this->uri(array('action'=>'managesettings')));
    $forms.=$objForm->show();

    $pageData.=$forms;

// set up exit link
    $objLink=new link($this->uri(array('action'=>'managesettings')),'email');
