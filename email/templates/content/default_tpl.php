...
// set up compose icon
    ...
    $composeIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'compose')),'notes');
...
//    $objTable->cellspacing='2';
    ...
    foreach($arrFolderList as $folder){
        //set up folder link
        $objLink=new link($this->uri(array('action'=>'gotofolder','folderId'=>$folder['id'])));
...
//    $objTable->cellspacing='2';
    ...
    if(empty($arrEmailListData)){
        ...
        foreach($arrEmailListData as $email){
            ...
            // set message icon
            $action=$this->uri(array('action'=>'gotomessage','folderId'=>$folderId,'routingId'=>$email['routing_id'],'filter'=>$filter));
    ...
    $objForm=new form('msgform',$this->uri(array('action'=>'messages','folderId'=>$folderId)));
...
// set up message table
    ...
    $objForm=new form('hiddenform',$this->uri(array('action'=>'gotomessage','folderId'=>$folderId,'routingId'=>$routingId,'filter'=>$filter)));
...
//    $objTable->cellspacing='2';
    ...
    if(empty($messageData)){
        ...
        // set up reply icon
        $array=array('date'=>$this->objDate->formatDate($emailData['date_sent']),'writer'=>$from);
        ...
        $replyIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'compose','userId'=>$emailData['sender_id'],'subject'=>"RE: ".$emailData['subject'],'message'=>$replyMessage,'emailId'=>$emailData['id'])),'reply');
        $icons=$replyIcon;
        ...
        $replyallIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'compose','userId'=>$strUserId,'subject'=>"RE: ".$emailData['subject'],'message'=>$replyMessage,'emailId'=>$emailData['id'])),'replyall');
        $icons.=$replyallIcon;
        ...
        $forwardIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'compose','subject'=>"FWD: ".$emailData['subject'],'message'=>$fwdMessage,'emailId'=>$emailData['id'])),'forward');
        $icons.=$forwardIcon;

        // set up resend icon
        if($messageData['sender_id']==$this->userId && $folderId=='init_3'){
            $objIcon->title=$resendLabel;
            $resendIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'gotofolder','folderId'=>'init_3','mode'=>'resend','emailId'=>$emailData['id'],'filter'=>$filter)),'resend');
            $icons.=$resendIcon;
        }else{
            $icons.='';
        }
        ...
        if($folderId=='init_4'){
            ...
            $restoreIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'gotofolder','folderId'=>$folderId,'mode'=>'restore','routingId'=>$messageData['id'],'filter'=>$filter)),'restore');
            $icons.=$restoreIcon;
        }else{
            $icons.='';
        }
...
//            $objTable->cellspacing='2';
            ...
            foreach($arrAttachments as $attachment){
                ...
                $objLink=new link($this->uri(array('action'=>'downloadfile','attachId'=>$attachment['id'])));
    ...
    $objIcon->extra='';
    $manageIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'managefolders','currentFolderId'=>$folderId)),'managefolders');

    // set up manage address books icon
    $objIcon->title=$manageBooksLabel;
    $objIcon->extra='';
    $booksIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'manageaddressbooks','folderId'=>$folderId)),'addressbook');

    // set up manage configs icon
    $objIcon->title=$manageSettingsLabel;
    $objIcon->extra='';
    $configIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'managesettings','currentFolderId'=>$folderId)),'mailconfig');
