...
// set up add icon
    ...
    $addIcon=$objIcon->getLinkedIcon($this->uri(array('action'=>'manageaddressbooks','mode'=>'add')),'add');
    ...
    if($arrBookList==FALSE && empty($arrContextList)){
        ...
        if(!empty($arrContextList)){
            foreach($arrContextList as $context){
                ...
                $groupId=$this->objGroupAdmin->getLeafId(array($context['contextCode']));
                ...
                // set up link
                $objLink=new link($this->uri(array('action'=>'addressbook','contextCode'=>$context['contextCode'],'menutext'=>$context['menutext'])),'email');
        ...
        if($arrBookList!=FALSE){
            foreach($arrBookList as $book){
                ...
                $editIcon=$objIcon->getEditIcon($this->uri(array('action'=>'manageaddressbooks','mode'=>'edit','bookId'=>$book['id'])));
                if($mode=='edit' && $bookId==$book['id']){
                    ...
                    // set up link
                    $objLink=new link($this->uri(array('action'=>'addressbook','bookId'=>$book['id'],'activeBook'=>$book['book_name'])),'email');
                    $objLink->link=$book['book_name'];
                    $bookName=$objLink->show();
                }

                // set up delete icon
                $deleteArray=array('action'=>'manageaddressbooks','mode'=>'delete','bookId'=>$book['id']);
...
// set up form
    $objForm=new form('bookform',$this->uri(array('action'=>'manageaddressbooks')));
...
// hidden element
    ...
    $objForm=new form('hiddenform',$this->uri(array('action'=>'manageaddressbooks')));
    $objForm->addToForm($hiddenInput);
    $hiddenForm=$objForm->show();
    $pageData.=$folderForm.$hiddenForm;

// set up exit link
    $objLink=new link($this->uri(array(''),'email'));
