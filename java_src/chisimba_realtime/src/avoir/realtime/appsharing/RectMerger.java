...
/*
	...
	private static Vector generateRandomSquares(Dimension dimArea, int iSideSize, int iCount){
		Rectangle[] rectsAll = TileMaker.makeTiles(dimArea, iSideSize);
	...
	public static void main(String[] args) throws Exception{
		...
		System.in.read();
		Vector vectMerged = mergeRects(testRects);
		System.out.println("after: [" + vectMerged.size() + " rects]");
		dumpRects(vectMerged);
	}
*/
// END TEST CODE

	public static Rectangle[] mergeRectangles(Rectangle[] rects){
		// make defensive copy of input
		Vector vectRects = new Vector();
		for(int i = 0; i < rects.length; i++){
			vectRects.addElement(new Rectangle(rects[i]));
		}
		Vector vectMerged = mergeRects(vectRects);
		Rectangle[] rectsReturn = new Rectangle[vectMerged.size()];
		for(int i = 0; i < rectsReturn.length; i++){
			rectsReturn[i] = (Rectangle)vectMerged.elementAt(i);
		}
		return rectsReturn;
	}

	private static Vector mergeRects(Vector vectRects){
...
//					System.out.println("...result is: " + merged);
					...
					vectRects = mergeRects(vectRects);
				}
			}
		}
		return vectRects;
	...
	private static boolean canMerge(Rectangle rect1, Rectangle rect2){
		int iArea = getRectArea(rect1) + getRectArea(rect2);
		return iArea == getRectArea(rect1.union(rect2));
	}
...
//
//	public static void dumpRects(Vector vectRects){
//		for(int i = 0; i < vectRects.size(); i++){
//			System.out.println(vectRects.elementAt(i));
//		}
//	}
