    ...
    if (count($studentInfo) > 0){
        ...
        if (count($results) > 0){
            ...
            for($j = (count($years) - 1); $j >= 0; $j--){
                ...
                $table->endRow();
                $resultsTabBox->addTab($years[$j],$years[$j],$table->show());
                $table = NULL;
            }
            $courseDisplay = $resultsTabBox->show(FALSE);
        }else{
            $courseDisplay = "<div class='noRecordsMessage'>".$objLanguage->languagetext('mod_financialaid_nomarks','financialaid')."</div>";
        }

        $appTabBox->addTab('courseDetails', $objLanguage->languagetext('word_course'), $courseDisplay);
        ...
        if (count($studentSchool) > 0){
            ...
            $appTabBox->addTab('matricDetails', $objLanguage->languagetext('word_matric'), $tables);
        }
